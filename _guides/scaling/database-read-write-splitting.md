...
excerpt: How to configure database read replicas to reduce primary load and improve scalability.
summary: This guide explains how to configure Cerb to split database traffic between 
  a primary (writer) and one or more read replicas. It covers the required constants 
  in framework.config.php, connection tuning options, post-write read behavior for avoiding 
  replication lag issues, legacy alias support, and troubleshooting tips.
topic: Scaling
jumbotron:
  title: Database read/write splitting
  breadcrumbs:

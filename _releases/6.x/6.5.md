...
jumbotron:
  title: "6.5"
excerpt: Cerb 6.5 is a major feature update released in September 2013 with 144+ new
  features and improvements from community feedback.
summary: This page details the release of Cerb version 6.5, a significant feature
  upgrade launched on September 20, 2013. The update introduces over 144 new features
  and improvements, heavily influenced by community feedback. Key enhancements include
  a comprehensive mobile plugin offering a platform-independent interface with features
  like mobile notifications, workspaces, and profiles, as well as iOS support. The
  update also advances Virtual Attendants, making them first-class records with enhanced
  capabilities such as real-time behavior execution, custom API requests, and improved
  usability. Additional features include a JIRA integration plugin, an activity log,
  enhanced calendar functionalities, and new custom field types. The release also
  focuses on improving mail management, localization, placeholders, scripting, and
  platform usability. Maintenance updates from 6.5.1 to 6.5.5 address minor fixes
  and usability enhancements based on user feedback. The page provides detailed instructions
  for upgrading and checking license eligibility for the update.

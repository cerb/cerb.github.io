...
jumbotron:
  title: "6.8"
excerpt: Cerb 6.8 is a major feature update released in September 2014 with 79+ new
  features and improvements from community feedback.
summary: 'This page details the release of Cerb version 6.8, a feature upgrade launched
  on September 17, 2014, which includes over 79 new features and improvements based
  on community feedback. Key enhancements include the introduction of @mentions for
  worker notifications, file bundles for managing attachments, and configurable HTML
  signatures for emails. The update also improves mobile functionality with #commands
  and @mentions, enhances workspace management, and refines search capabilities for
  better relevance and result limits. Virtual Attendants receive significant upgrades
  with new placeholders, scripting syntax help, and expanded behavior variables. Profiles
  now include improved mail history and merging capabilities, while snippets benefit
  from app ownership and placeholder enhancements. The activity log, calendar, knowledgebase,
  and support center also see various improvements. The platform update includes browser
  tab labels and enhanced comment navigation. Several maintenance updates (6.8.1 to
  6.8.8) address minor features, fixes, and usability enhancements, ensuring a more
  robust and user-friendly experience. Users are advised to back up before upgrading
  and can check their eligibility for a free update based on their license expiration
  date.'

...
jumbotron:
  title: "7.3"
excerpt: Cerb 7.3 is a major feature update released in February 2017 with 178+ new
  features and improvements from community feedback.
summary: This page details the release notes for Cerb version 7.3, a feature upgrade
  launched on February 6, 2017. The update includes over 178 new features and improvements
  based on community feedback. Key highlights include the renaming of Virtual Attendants
  to bots, the introduction of Connected Accounts for streamlined plugin credentials,
  and enhanced bot functionalities such as conversational behaviors, behavior fragments,
  and improved access control. The release also introduces significant enhancements
  in search capabilities, including filtered fulltext search and deep quick search
  options across various record types. Additionally, the update improves performance
  in ticket worklists, introduces new worklists and profile pages for attachments,
  comments, and roles, and enhances usability features across the platform. The release
  requires PHP 5.5+ and the `openssl` extension, and it emphasizes the importance
  of making backups before upgrading. The page also provides detailed instructions
  for upgrading and information on licensing and subscription updates.

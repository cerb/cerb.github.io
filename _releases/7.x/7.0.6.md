...
jumbotron:
  title: 7.0.6
excerpt: Cerb 7.0.6 is a maintenance update released in October 2015 with 10 fixes
  and minor features from community feedback.
summary: This page details the release notes for Cerb version 7.0.6, a maintenance
  update released on October 14, 2015. The update includes 10 improvements based on
  community feedback from the 7.0 update. Key fixes include resolving issues with
  the Web-API for workspace pages, displaying HTML messages when storage objects are
  missing, and addressing a potential XSS vulnerability in the snippets chooser. Additional
  improvements involve code cleanup, fixing stale group membership information in
  message worklists, implementing a new filter for message worklists, and resolving
  issues with Microsoft Office formatted HTML messages. The update also addresses
  translation issues in MySQL strict mode and corrects placeholder conversions in
  dashboard widgets. Instructions for upgrading to this version are provided.

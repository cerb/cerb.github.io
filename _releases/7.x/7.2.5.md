...
jumbotron:
  title: 7.2.5
excerpt: Cerb 7.2.5 is a maintenance update released in January 2017 with 26 fixes
  and minor features from community feedback.
summary: This page details the Cerb 7.2.5 maintenance update released on January 9,
  2017, which includes 26 fixes and minor features based on community feedback from
  the 7.2 update. Key improvements focus on resolving issues with worklists and search
  functionalities, such as fixing duplicate ticket search results, enhancing quick
  search filters with new `id:` filters across various worklists, and addressing Elasticsearch
  integration inefficiencies. Additional fixes include improvements in bot behavior
  simulation, email usability in Markdown mode, and correcting display issues on group
  cards. The update also resolves issues with search indexing for worker full names
  and permissions on message worklists.

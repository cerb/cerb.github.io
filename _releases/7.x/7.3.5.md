...
jumbotron:
  title: 7.3.5
excerpt: Cerb 7.3.5 is a maintenance update released in March 2017 with 9 fixes and
  minor features from community feedback.
summary: This page details the Cerb 7.3.5 maintenance update released on March 13,
  2017, which includes nine fixes and minor features based on community feedback from
  the 7.3 update. Key improvements include resolving issues with HTML templates stripping
  the ID attribute, enhancing bot behavior visibility and email sending actions, fixing
  ticket count filters on bucket cards, and modifying the installer to remove a problematic
  registration survey. Additionally, new features such as a quick search filter for
  ticket worklists, improved worker profile visibility on ticket worklists, a new
  text quoting filter for bot scripting, and enhanced message reply quoting functionality
  are introduced.

...
jumbotron:
  title: 7.3.3
excerpt: Cerb 7.3.3 is a maintenance update released in February 2017 with 19 fixes
  and minor features from community feedback.
summary: This page details the Cerb 7.3.3 maintenance update released on February
  28, 2017, which includes 19 fixes and minor features based on community feedback
  from the 7.3 update. Key improvements include enhanced usability for sticky notes
  on ticket profiles, better handling of email actions in bots, improved attachment
  visibility in ticket profiles, and fixes for installer issues with PHP and MariaDB.
  The update also addresses various usability and functionality issues across different
  components such as bots, worklists, and the debug status report. Additionally, it
  introduces new features like a 'Simulator' button for bot behavior cards and improved
  error reporting for mail drafts.

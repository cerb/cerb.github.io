...
jumbotron:
  title: 7.3.8
excerpt: Cerb 7.3.8 is a maintenance update released in April 2017 with 19 fixes and
  minor features from community feedback.
summary: This page details the release notes for Cerb version 7.3.8, a maintenance
  update launched on April 3, 2017. The update addresses 19 fixes and minor enhancements
  based on community feedback from the 7.3 update. Key improvements include resolving
  an API issue with ticket replies, enhancing bot functionality with proper notification
  for `@mentions`, and improving usability with help instructions for empty behavior
  worklists and bot cards. The update also refines validation for the 'Owner' field,
  supports list-based behavior variables in bulk updates, and introduces several scripting
  enhancements for bots, including new functions for generating avatar images and
  URLs. Additionally, the update implements a sandbox security policy for Twig template
  scripting to enhance security.

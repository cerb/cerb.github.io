...
jumbotron:
  title: 7.0.1
excerpt: Cerb 7.0.1 is a maintenance update released in June 2015 with 12 fixes and
  minor features from community feedback.
summary: This page details the Cerb 7.0.1 maintenance update released on June 9, 2015,
  which includes 12 improvements based on community feedback from the 7.0 update.
  Key enhancements include changes to mail parsing to prevent unnecessary attachments,
  usability improvements in worklists and watcher functionalities, adjustments to
  group responsibilities, and fixes for issues related to mail relay, duplicate queries,
  and ticket conversation preferences. Additionally, new options for replying to specific
  recipients in ticket messages and enhancements in Virtual Attendant behaviors for
  email actions are introduced. The update also ensures consistency in the interface
  for recommended and watching buttons across different views.

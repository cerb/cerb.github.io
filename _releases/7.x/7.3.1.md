...
jumbotron:
  title: 7.3.1
excerpt: Cerb 7.3.1 is a maintenance update released in February 2017 with 29 fixes
  and minor features from community feedback.
summary: This page details the release notes for Cerb version 7.3.1, a maintenance
  update launched on February 15, 2017. The update addresses 29 fixes and minor enhancements
  based on community feedback from the 7.3 update. Key improvements include resolving
  issues with the 'Comment' button on profile pages, setting 'InnoDB' as the default
  MySQL storage engine, and fixing access errors on ticket profiles. Enhancements
  also cover worker autocomplete suggestions, S3 storage engine configurations, and
  usability improvements for attachments, comments, and the global search menu. The
  update also improves performance for privilege checks on attachments and introduces
  streaming support for file downloads. Additionally, it addresses compatibility issues
  with older browsers and enhances the usability of activity logs, notifications,
  and worklists.

...
jumbotron:
  title: 10.3.11
excerpt: Cerb 10.3.11 is a maintenance update released in July 2023 with 5 improvements
  from community feedback.
summary: This page details the Cerb 10.3.11 maintenance update released on July 17,
  2023. It highlights five minor features and fixes based on community feedback from
  the 10.3 update. Key updates include enhancements to `interaction.worker` automations
  with new client placeholders, a fix for an issue with HTTP request headers in automations,
  improvements in handling the `members` field when updating group records, and an
  improved scrollbar style for dark mode in browsers. Additionally, a security update
  was made to the `league/oauth2-server` library in response to a vulnerability notice.
  Instructions for upgrading are also provided.

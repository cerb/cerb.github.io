...
jumbotron:
  title: "10.1"
excerpt: Cerb 10.1 is a major feature update released in September 2021 with 114 improvements
  from community feedback.
summary: This page details the release of Cerb 10.1, a feature upgrade launched on
  September 10, 2021, incorporating over 114 new features and improvements based on
  community feedback. Key enhancements include advanced search capabilities with no
  record limits, fulltext negation, and improved performance and usability. Interactions
  have been refined with new continuation options and improved simulation and usability.
  Automation features have been expanded with sequential set actions, optional annotations,
  and enhanced error reporting. New automation events for mail handling have been
  introduced, such as mail.draft, mail.draft.validate, mail.received, mail.send, and
  mail.sent. Data queries now support grouping by distinct values, and mail functionalities
  have been enhanced with custom fieldsets in drafts, keyboard shortcuts, and the
  ability to download all attachments as a ZIP. Time tracking has been improved with
  persistent timers, the ability to resume entries, and tracking in seconds. The release
  notes emphasize the importance of backups before upgrading and provide compatibility
  requirements for PHP and MySQL. The page also includes a comprehensive changelog
  detailing all updates and improvements.
...
* **[Automations/Scripting]** In automation scripting, added a [strip_lines(prefixes)](/docs/scripting/filters/#strip_lines) filter. This removes lines with the given prefixes. This is particularly useful for removing quoted text in compose/reply behaviors.

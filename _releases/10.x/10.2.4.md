...
jumbotron:
  title: 10.2.4
excerpt: Cerb 10.2.4 is a maintenance update released in April 2022 with 12 improvements
  from community feedback.
summary: This page details the Cerb 10.2.4 maintenance update released on April 29,
  2022, which includes 12 minor features and fixes based on community feedback from
  the 10.2 update. Key additions include new automation scripting capabilities such
  as a CSV parsing filter and a KATA document parsing command. Changes involve simplifications
  in automation actions, such as no longer requiring the `output:` key for certain
  actions and allowing ticket creation with only a `bucket_id:` field. Fixes address
  issues with downloading attachments on Windows Server, PHP 8.1 compatibility, and
  installer validation. A security update was also made by updating the async.js dependency
  to address a security notice.

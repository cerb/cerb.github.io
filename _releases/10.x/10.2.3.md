...
jumbotron:
  title: 10.2.3
excerpt: Cerb 10.2.3 is a maintenance update released in April 2022 with 31 improvements
  from community feedback.
summary: This page details the Cerb 10.2.3 maintenance update released on April 6,
  2022, which includes 31 minor features and fixes based on community feedback from
  the 10.2 update. Key additions include new automation scripting filters and quick
  search filters for worker worklists. Changes involve improved autocomplete for search
  queries and enhanced error messages for CSV imports. Several bugs were fixed, such
  as issues with record changes, mail routing, and record merging. Security enhancements
  were made, including improved input validation, path traversal fixes, and stricter
  Content-Security-Policy enforcement. The update also addresses deprecation warnings
  in PHP 8.1 and improves user experience with clearer error messages on profile and
  setup pages.

...
jumbotron:
  title: 10.4.1
excerpt: Cerb 10.4.1 is a maintenance update released in August 2023 with 42 improvements
  from community feedback.
summary: This page details the release notes for Cerb version 10.4.1, a maintenance
  update launched on August 30, 2023. The update includes 42 minor features and fixes
  based on community feedback from the 10.4 update. Key additions include enhancements
  to automation events, such as triggering record changes upon deletion and introducing
  a vector similarity command for semantic search. The update also improves worklist
  functionalities, including export and import capabilities, and introduces new connected
  services like OpenAI and Hugging Face. Several user experience improvements are
  made, such as lazy loading of images and expanded explore modes for various worklists.
  The update also addresses numerous bug fixes, including issues with notification
  deletions, cache invalidation, and compatibility with Elasticsearch 8.x. Additionally,
  the update removes the Twitter package from connected services.
...
* **[Automations/Events]** The [record.changed](/docs/automations/events/record.changed/) automation event is now triggered when records are deleted. The event has a new `change_type` input with a value of `created`, `updated`, or `deleted`. For `created` the `was_record_*` dictionary is empty and `record_*` is the current state. For `deleted` the `was_record_*` and `record_*` are identical as the final state (this avoids existing listeners seeing deletion as changing every field value to blank). This behavior can be temporarily disabled with the `APP_OPT_RECORD_CHANGED_NO_DELETE` config option. Thanks to [1Password](https://1password.com/) for the feature request!
...
* **[Search/Elasticsearch]** Fixed an issue with Elasticsearch 8.x compatibility as a full-text search schema (e.g. message content). Thanks to [@nicholsk18](https://github.com/nicholsk18/) for the [patch](https://github.com/jstanden/cerb/pull/1737)!

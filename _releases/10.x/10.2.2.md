...
jumbotron:
  title: 10.2.2
excerpt: Cerb 10.2.2 is a maintenance update released in March 2022 with 15 improvements
  from community feedback.
summary: This page details the release notes for Cerb version 10.2.2, a maintenance
  update launched on March 18, 2022. The update includes 15 minor features and fixes
  based on community feedback from the previous 10.2 update. Key enhancements include
  the ability to create attachments from automation with improved memory efficiency,
  the addition of custom stop words to MySQL Fulltext search schemas for better search
  performance, and validation of script and policy syntax in the automation editor
  before running simulations. Several performance optimizations and bug fixes are
  also highlighted, such as improved sorting and subtotaling in worklists, resolving
  issues with ticket record merging, and enhancing the security of website interaction
  portals. The update addresses various issues across different components like project
  boards, tasks, and support center functionalities, ensuring a more efficient and

...
jumbotron:
  title: 10.0.2
excerpt: Cerb 10.0.2 is a maintenance update released in June 2021 with 8 minor features
  and fixes from community feedback.
summary: This page details the Cerb 10.0.2 maintenance update released on June 1,
  2021, which includes eight minor features and fixes based on community feedback
  from the 10.0 update. Key updates include fixes to the 'Interaction Toolbar' profile
  widgets and legacy behaviors, temporary restoration of certain mail behavior events
  pending replacement with automations in the 10.1 update, and improvements to the
  user interface, such as alignment issues with split buttons. Additionally, the update
  addresses issues with the 'Create Reminder' action in legacy bot behaviors and simplifies
  the reminders functionality by transitioning from behaviors to automation events
  for notifications. Deprecated chat bot behaviors are now easier to transition to
  interaction automations, with plans to remove chat bots in favor of automations
  in version 11.0.

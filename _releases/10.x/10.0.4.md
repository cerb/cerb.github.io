...
jumbotron:
  title: 10.0.4
excerpt: Cerb 10.0.4 is a maintenance update released in June 2021 with 10 minor features
  and fixes from community feedback.
summary: This page details the Cerb 10.0.4 maintenance update released on June 22,
  2021, which includes ten minor features and fixes based on community feedback from
  the 10.0 update. Key improvements include fixes for custom toolbar alignment and
  style consistency, automation input handling, record link filter display, and record
  dictionary key expansion. Additionally, enhancements were made to card toolbar interactions,
  worklist subtotal grouping, ticket worklist search filter autocompletion, and a
  new `array_count_values()` function was added for automation scripting. The update
  addresses various user-reported issues and feature requests, ensuring a more seamless

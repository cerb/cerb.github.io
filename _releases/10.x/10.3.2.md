...
jumbotron:
  title: 10.3.2
excerpt: Cerb 10.3.2 is a maintenance update released in October 2022 with 16 improvements
  from community feedback.
summary: This page details the Cerb 10.3.2 maintenance update released on October
  27, 2022, which includes 16 minor features and fixes based on community feedback
  from the 10.3 update. Key additions include new automation events for data queries,
  enhancements in automation scripting, and improved functionality for Chart KATA
  widgets, such as exporting data in JSON or CSV formats. Changes also involve performance
  improvements in data query subtotals and enhanced error messaging for file uploads
  in website interactions. Fixes address issues with SVG file uploads, Chart KATA
  widget restrictions, and visual elements in sheet widgets. The update aims to enhance
  integration capabilities, data handling, and user interface interactions within
  Cerb.
...
* **[Automations/Scripting]** In automation scripting, added a [\|parse_user_agent](/docs/scripting/filters/#parse_user_agent) filter. This parses a user agent string and returns an array with keys for: `platform`, `browser`, and `version`.

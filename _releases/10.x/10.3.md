...
jumbotron:
  title: "10.3"
excerpt: Cerb 10.3 is a major feature update released in September 2022 with 98 improvements
  from community feedback.
summary: This page details the release notes for Cerb version 10.3, a feature upgrade
  launched on September 28, 2022. The update introduces over 98 new features and improvements
  based on community feedback. Key highlights include enhancements to the automation
  editor with a new 'Change History' feature, the addition of complex chart creation
  on workspace dashboards, and improvements in automation scripting and file handling.
  The release also updates several platform dependencies, improves user experience
  with better UI elements, and enhances performance in record searches. Security updates
  include improvements to cookie security, the addition of a default Content-Security-Policy
  header, and updates to template engines due to security advisories. The release
  also removes support for the Sphinx search engine and several outdated dependencies.
  Users are advised to ensure their systems meet the new PHP and MySQL requirements
  before upgrading.
...
* **[Automations/Scripting]** In automation scripting, added a new [date_lerp(date_range,unit,step,limit)](/docs/scripting/functions/#date_lerp) function for interpolating the timestamps between two dates with the given `unit` (`minute`, `day`, `week`, `month`) and `step` (1, 5, 10).

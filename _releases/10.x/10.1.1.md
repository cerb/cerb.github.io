...
jumbotron:
  title: 10.1.1
excerpt: Cerb 10.1.1 is a maintenance update released in September 2021 with 14 minor
  features and fixes from community feedback.
summary: This page details the Cerb 10.1.1 maintenance update released on September
  20, 2021, which includes 14 minor features and fixes based on community feedback
  from the 10.1 update. Key enhancements focus on automation scripting, such as new
  filters for array manipulation (column, filter, map, reduce, values) and a function
  for calculating time elapsed using calendar availability. Improvements also include
  error handling in template scripting, search filter generation for worker-based
  custom fields, and data query enhancements for worker custom fields. Additionally,
  the update addresses issues in interaction sheets, ticket report calculations, and
  error checking in KATA automations. It also fixes problems in worker interactions

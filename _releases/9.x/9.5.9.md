...
jumbotron:
  title: 9.5.9
excerpt: Cerb 9.5.9 is a maintenance update released in July 2020 with 8 minor features
  and fixes from community feedback.
summary: This page details the Cerb 9.5.9 maintenance update released on July 30,
  2020, which includes eight minor features and fixes based on community feedback
  from the 9.5 update. Key improvements include resolving issues with classifier training
  examples containing HTML tags, enhancing user experience in form interactions with
  a 'me' shortcut for worker record selection, and fixing logging issues for email
  template deletions. Additionally, it addresses problems with 'message/rfc822' attachments
  in Outlook, increases the truncation threshold for record names in the worklist
  subtotals sidebar, and corrects issues with email formatting and template previews.
  The update also fixes permission checks for non-admin workers broadcasting replies
  in bulk ticket updates.

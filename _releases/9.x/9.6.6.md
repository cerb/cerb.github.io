...
jumbotron:
  title: 9.6.6
excerpt: Cerb 9.6.6 is a maintenance update released in October 2020 with 7 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.6.6 maintenance update released on October 26,
  2020, which includes seven minor features and fixes based on community feedback
  from the 9.6 update. Key improvements include fixes for autocompletion issues in
  custom record and email signature search queries, adjustments to string validation
  affecting fields like 'DOB' on contacts, and usability enhancements in the reply
  editor and ticket profiles. Additionally, the update addresses issues with contact
  and organization creation processes and improves the visibility of bucket totals
  in ticket worklists. The update reflects contributions and suggestions from users
  such as @mryanb, Flexibits, and 1Password.

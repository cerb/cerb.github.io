...
jumbotron:
  title: 9.0.3
excerpt: Cerb 9.0.3 is a maintenance update released in August 2018 with 15 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.0.3 maintenance update released on August 27,
  2018, which includes 15 minor features and fixes based on community feedback from
  the 9.0 update. Key improvements include enhanced logo management in branding settings,
  new export options for dashboard widgets, improved popup functionality for better
  user experience, and fixes for project board widget issues. The update also introduces
  performance enhancements for workspace widgets, usability improvements for profile
  dashboards, and new Ajax endpoints for ticket management. Additionally, it addresses
  issues with HTML message links and reintroduces the "Always read all" option in
  mail preferences.

...
jumbotron:
  title: 9.4.3
excerpt: Cerb 9.4.3 is a maintenance update released in December 2019 with 12 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.4.3 maintenance update released on December
  19, 2019, which includes 12 minor features and fixes based on community feedback
  from the 9.4 update. Key improvements include fixes to the user interface for mail
  and comments, enhancements to date handling in sheets, and updates to record handling
  for email addresses and contacts. The update also addresses issues with workspace
  page sorting, role member cache updates, form interaction simulations, and bot behavior
  execution. Additionally, it improves the handling of comments with attachments and
  the creation of tickets in bot behaviors.

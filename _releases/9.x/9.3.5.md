...
jumbotron:
  title: 9.3.5
excerpt: Cerb 9.3.5 is a maintenance update released in October 2019 with 17 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.3.5 maintenance update released on October 10,
  2019, which includes 17 minor features and fixes based on community feedback from
  the 9.3 update. Key updates include resolving a rare issue with ticket mask generation,
  fixing LDAP plugin migration issues, enhancing the API to open tickets without sending
  emails, and improving bot scripting and form interactions. Additional improvements
  address mail composition, developer tools, user interface messages, scheduled behavior
  worklists, and storage profile configurations. The update also introduces new features
  like an icons reference page for developers and enhanced management of mail signature
  attachments.

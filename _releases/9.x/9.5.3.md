...
jumbotron:
  title: 9.5.3
excerpt: Cerb 9.5.3 is a maintenance update released in April 2020 with 16 minor features
  and fixes from community feedback.
summary: This page details the release notes for Cerb version 9.5.3, a maintenance
  update issued on April 13, 2020. It addresses 16 minor features and fixes based
  on community feedback from the 9.5 update. Key improvements include fixes to the
  API, particularly the `/tickets/compose.json` endpoint, enhancements in worker mention
  handling, and adjustments to S3 storage profiles. The update also resolves issues
  with session management in the REST API, workspace tab ordering, webhook listener
  creation, and MySQL compatibility. Additionally, new functionalities like the `cerb_extract_uris()`
  function in bot scripting and a customizable 'Draft Generator' widget for ticket

...
jumbotron:
  title: 9.1.7
excerpt: Cerb 9.1.7 is a maintenance update released in February 2019 with 8 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.1.7 maintenance update released on February
  26, 2019, which includes eight minor features and fixes based on community feedback
  from the 9.1 update. Key improvements include resolving issues with custom fields
  for files, enhancing the record API, updating workspace worklists and snippets,
  and improving real-time updates for dashboard widgets. Additionally, the update
  introduces changes to data queries for better handling of date aggregations, and
  transitions the database connection to 'utf8mb4' encoding for broader character
  support. The update also provides instructions for upgrading to this version.

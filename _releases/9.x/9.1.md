...
jumbotron:
  title: "9.1"
excerpt: Cerb 9.1 is a major feature update released in December 2018 with 73 improvements
  from community feedback.
summary: This page details the release of Cerb 9.1, a feature upgrade launched on
  December 27, 2018, which introduces over 73 new features and improvements based
  on community feedback. Key highlights include enhanced security features such as
  Single Sign-On (SSO) integration, Multi-Factor Authentication (MFA), and simplified
  login processes. The update also introduces connected services, allowing for more
  flexible and integrated service provider connections, including Amazon Web Services,
  OAuth2, and SAML, among others. The API now supports OAuth2 authentication, and
  geolocation features have been expanded with new custom fields and map widgets.
  Custom fields can now be extended with plugins, and dashboards have been simplified
  with YAML-based filters. The release also improves bot scripting, record usability,
  and platform dependencies, while retiring several plugins in favor of integrated
  solutions. The update requires PHP 7.0+ and MySQL 5.6+, and emphasizes the importance
  of making a backup before upgrading.

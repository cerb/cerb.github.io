...
jumbotron:
  title: 9.4.7
excerpt: Cerb 9.4.7 is a maintenance update released in January 2020 with 24 minor
  features and fixes from community feedback.
summary: This page details the release notes for Cerb version 9.4.7, a maintenance
  update issued on January 10, 2020. The update includes 24 minor features and fixes
  based on community feedback from the 9.4 update. Key improvements address issues
  in mail composition, installer compatibility, record broadcasting, user experience
  enhancements in merging records, and single sign-on login processes. The update
  also introduces new user preferences, optimizes bot behavior actions, and resolves
  performance issues related to worker and group editors. Additionally, it refines
  the user interface for composing and replying to emails, including adjustments to
  encryption options and keyboard shortcuts for snippets.

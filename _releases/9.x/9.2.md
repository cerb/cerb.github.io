...
jumbotron:
  title: "9.2"
excerpt: Cerb 9.2 is a major feature released in March 2019 with 118 improvements
  from community feedback.
summary: This page details the release of Cerb 9.2, a feature upgrade launched on
  March 8, 2019, incorporating over 118 new features and improvements based on community
  feedback. Key highlights include enhancements to the Package Library, which now
  allows broader use beyond administrators, enabling workers to select from pre-approved
  templates to automate workflows. The update introduces new libraries for bot behaviors,
  calendars, connected services, and various workspace components, significantly streamlining
  processes like creating project boards and dashboards. The release also improves
  role management with automatic membership queries and flexible access controls for
  role-owned records. Dashboards now feature interactive charts that allow users to
  click and search underlying data. Bots have been enhanced with customizable record
  editors and improved HTTP request handling. The update also includes performance
  improvements, such as faster record lookups and enhanced data query capabilities,
  including support for emoji characters and new visualization options. The release
  notes emphasize the importance of backing up before upgrading and outline the technical

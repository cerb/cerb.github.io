...
jumbotron:
  title: 9.3.2
excerpt: Cerb 9.3.2 is a maintenance update released in July 2019 with 25 minor features
  and fixes from community feedback.
summary: This page details the release notes for Cerb version 9.3.2, a maintenance
  update launched on July 11, 2019. The update includes 25 minor features and fixes
  based on community feedback from the 9.3 update. Key improvements focus on enhancing
  the usability of bot behaviors, such as organizing events into logical categories
  and grouping global actions in submenus. New actions like 'Create comment', 'Create
  notification', and 'Send email' are now universally available in bot behaviors.
  The update also addresses various bugs, including a caching issue in data query
  widgets and a bug in bot behaviors related to the 'Exit' action. Security enhancements
  include automatic HTML escaping for placeholders in dashboard widgets. Additionally,
  the records API has been improved with new fields and functionalities, such as managing
  ticket participants and unlinking records. The update also fixes issues related

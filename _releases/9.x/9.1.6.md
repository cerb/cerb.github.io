...
jumbotron:
  title: 9.1.6
excerpt: Cerb 9.1.6 is a maintenance update released in February 2019 with 14 minor
  features and fixes from community feedback.
summary: This page details the release notes for Cerb version 9.1.6, a maintenance
  update launched on February 19, 2019. The update includes 14 minor features and
  fixes based on community feedback from the 9.1 update. Key improvements focus on
  optimizing search performance for ticket and message records, addressing potential
  deadlock issues during full-text searches, and enhancing data query functionalities
  with new formatting expressions. Additionally, the update resolves issues with API
  mail template retrieval, time tracking record inconsistencies, opportunity placeholder
  dictionaries, worklist export errors, and role editor duplications. The update aims
  to improve system efficiency and user experience by addressing these specific issues.

...
jumbotron:
  title: 9.6.4
excerpt: Cerb 9.6.4 is a maintenance update released in September 2020 with 7 minor
...
summary: This page details the release of Cerb version 9.6.4, a maintenance update
  launched on September 30, 2020. It introduces seven minor features and fixes based
  on community feedback from the 9.6 update. Key enhancements include new bot scripting
  functions and filters such as `|indent(marker, fromLine)`, `array_matches(values,
  patterns)`, `|csv`, and `array_extract_keys()`, which improve text formatting, pattern
  matching, and data extraction capabilities. Additionally, the update addresses an
  issue in the activity log related to missing translations for the `record.delete`
  event and provides a new option in ticket worklists for bulk updates to prevent
  modification of the updated timestamp. A fix was also made to ensure consistent
  behavior in 'Message added to ticket' events across different bot behaviors.

...
jumbotron:
  title: 9.5.11
excerpt: Cerb 9.5.11 is a maintenance update released in August 2020 with 5 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.5.11 maintenance update released on August 7,
  2020, which includes five minor features and fixes based on community feedback from
  the 9.5 update. Key improvements include automatic trimming of trailing slashes
  in CORS URLs for 'Conversation Bot' portals, a fix for comment record validation
  issues, automatic recipient suggestions when resuming drafts with an organization
  set, inclusion of the `uri` field when exporting bot behaviors, and enhanced import
  capabilities for packages with bots by including URIs in behaviors.

...
jumbotron:
  title: 9.6.2
excerpt: Cerb 9.6.2 is a maintenance update released in August 2020 with 20 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.6.2 maintenance update released on August 21,
  2020, which includes 20 minor features and fixes based on community feedback from
  the 9.6 update. Key improvements include fixes to mail signature commands, enhancements
  in displaying custom field labels, and updates to HTML email signature editing.
  The update also introduces new functionalities in bots and dashboards, such as access
  to placeholders for the current worker and widget configurations. Additionally,
  several enhancements were made to data queries, including new grouping options for
  date fields and improved autocomplete features. The update also adds new filters
  for advanced date searches and enhances HTML mail templates with additional placeholders
  and styling options.

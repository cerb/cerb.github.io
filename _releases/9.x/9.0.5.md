...
jumbotron:
  title: 9.0.5
excerpt: Cerb 9.0.5 is a maintenance update released in September 2018 with 11 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.0.5 maintenance update released on September
  25, 2018, which includes 11 minor features and fixes based on community feedback
  from the 9.0 update. Key improvements include the relocation of bundled plugins
  to a new directory for better scalability in cloud environments, fixes to calendar
  widgets on dashboards, and enhancements to the print functionality of profile pages.
  The update also improves the Support Center portal with new filters and consistent
  status labels, displays 'Cc:' recipients on ticket cards, and resolves issues with
  the community portal's proxy script. Additionally, it introduces a shortcut to 'Settings'
  in the worker menu, corrects the saving of the "Date of Birth" field in records,
  and ensures immediate reflection of name changes on workspace tabs.

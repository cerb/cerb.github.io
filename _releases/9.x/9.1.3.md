...
jumbotron:
  title: 9.1.3
excerpt: Cerb 9.1.3 is a maintenance update released in January 2019 with 11 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.1.3 maintenance update released on January 22,
  2019, which includes 11 minor features and fixes based on community feedback from
  the 9.1 update. Key improvements include enhancements to dashboard date filters,
  database logging for MySQL errors, ticket merging processes, and the package exporter
  script for time tracking entries. The update also addresses issues with worklist
  copying, introduces a multiple selection option for dashboard filters, and improves
  JIRA integration by storing 'Status' and 'Type' fields as text. Additionally, it
  provides developers with the ability to enable or disable the event system procedurally,
  particularly useful for importing historical data without triggering notifications
  or bot behaviors. The update also fixes a setup issue related to form saving in

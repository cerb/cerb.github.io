...
jumbotron:
  title: 9.1.4
excerpt: Cerb 9.1.4 is a maintenance update released in February 2019 with 11 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.1.4 maintenance update released on February
  5, 2019, which includes 11 minor features and fixes based on community feedback
  from the 9.1 update. Key improvements include resolving issues with the email parser,
  data queries, and bot scripting. The update also enhances dashboard functionality,
  such as pie chart widgets and filter handling, and addresses problems with orphaned
  bot behaviors, package validation, and the legacy printing plugin. Additionally,
  it fixes issues related to the upgrade process and dashboard editing. The page provides

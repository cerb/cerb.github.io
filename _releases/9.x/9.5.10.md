...
jumbotron:
  title: 9.5.10
excerpt: Cerb 9.5.10 is a maintenance update released in August 2020 with 11 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.5.10 maintenance update released on August 5,
  2020, which includes 11 minor features and fixes based on community feedback from
  the 9.5 update. Key improvements include bug fixes in sheets for date formatting,
  enhancements in the records API for ticket creation, corrections for handling invalid
  email addresses, and updates to contact record creation with organization name options.
  Additionally, the update addresses issues in project board behaviors, improves user
  experience in worklists, and refines form interaction prompts. Other notable changes
  involve adjustments to mail transport testing timeouts and ensuring the organization
  field is correctly applied when composing mail.

...
jumbotron:
  title: 9.5.6
excerpt: Cerb 9.5.6 is a maintenance update released in May 2020 with 17 minor features
  and fixes from community feedback.
summary: This page details the Cerb 9.5.6 maintenance update released on May 22, 2020,
  which includes 17 minor features and fixes based on community feedback from the
  9.5 update. Key improvements include fixes to Twitter message worklists, bot behaviors,
  and mail reply functionalities. Enhancements also cover user experience improvements
  like new keyboard shortcuts for toggling watchers, the ability to unset the gender
  field in bulk updates, and the option to select specific mail templates when composing
  or replying to emails. Additional updates address issues with data queries, Twitter
  character count displays, support center file attachments, bot behavior packages,
  card widget paging, and PGP message decryption. The update also introduces the ability
  to customize worklist columns in choosers and set the lifespan of OAuth2 access

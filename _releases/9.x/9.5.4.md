...
jumbotron:
  title: 9.5.4
excerpt: Cerb 9.5.4 is a maintenance update released in April 2020 with 12 minor features
  and fixes from community feedback.
summary: This page details the Cerb 9.5.4 maintenance update released on April 20,
  2020, which includes 12 minor features and fixes based on community feedback from
  the 9.5 update. Key improvements include enhancements to mail handling, such as
  improved quote deletion and snippet insertion, and the addition of a Markdown to
  HTML filter in bot scripting. The update also addresses issues with scheduled behavior
  worklists, snippet handling in various browsers, image insertion in emails, and
  mail relay configurations. Additionally, it includes fixes for card widget configuration
  and JavaScript handling in the platform's development tools.

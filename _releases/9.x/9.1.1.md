...
jumbotron:
  title: 9.1.1
excerpt: Cerb 9.1.1 is a maintenance update released in January 2019 with 9 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.1.1 maintenance update released on January 9,
  2019, which includes nine minor features and fixes based on community feedback from
  the 9.1 update. Key improvements include resolving an API key expansion issue, adding
  MySQL version checks for SPATIAL index support in the installer and updater, and
  fixing bot behavior script issues with temporary variables. Additionally, it addresses
  mail-related issues such as draft creation during ticket replies and enhances the
  user experience in mail routing configuration. The update also introduces a new
  'thirds' layout for workspace dashboards, improving responsiveness and usability.
  Furthermore, API usability is enhanced by consistently formatting search response
  keys as numbers.

...
jumbotron:
  title: 9.6.3
excerpt: Cerb 9.6.3 is a maintenance update released in September 2020 with 5 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.6.3 maintenance update released on September
  7, 2020, which includes five minor features and fixes based on community feedback
  from the 9.6 update. Key improvements include resolving an issue with the watchers
  popup inaccurately counting idle sessions as active, enhancing the user experience
  when replying to emails by ensuring a blank line after signatures, and optimizing
  the display of ticket profile pages on narrow screens by adjusting indentation and
  toolbar positioning. Additionally, it addresses a problem with comment widgets on
  non-ticket profiles where threaded comments were not initially visible, and fixes
  an issue with custom email headers in 'Before message sent by worker' behaviors.

...
jumbotron:
  title: 9.3.8
excerpt: Cerb 9.3.8 is a maintenance update released in November 2019 with 20 minor
  features and fixes from community feedback.
summary: This page details the release notes for Cerb version 9.3.8, a maintenance
  update launched on November 24, 2019. The update includes 20 minor features and
  fixes based on community feedback from the 9.3 update. Key improvements include
  updates to bot scripting documentation, increased file size limits for profile pictures,
  enhancements in worker worklists, and dynamic options in sheet card columns. The
  update also addresses issues with bot behaviors, HTML message links, and form interactions.
  Additionally, it introduces autocomplete suggestions for various query filters in
  worklists, enhancing the user experience across different record types.

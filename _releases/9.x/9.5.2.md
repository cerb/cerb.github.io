...
jumbotron:
  title: 9.5.2
excerpt: Cerb 9.5.2 is a maintenance update released in March 2020 with 8 minor features
  and fixes from community feedback.
summary: This page details the Cerb 9.5.2 maintenance update released on March 27,
  2020, which includes eight minor features and fixes based on community feedback
  from the 9.5 update. Key improvements include resolving issues with popups lacking
  focusable elements, preserving 'disabled' or 'simulator' nodes in bot behavior imports,
  and fixing script linefeed issues in bot behavior scheduling. Usability enhancements
  were made for email address entry and form interaction widgets, enabling autocompletion
  for quicker selections. Additionally, issues with bulk updates on worklists and
  spam reporting on ticket worklists were addressed. The update reflects Cerb's commitment
  to refining user experience and functionality based on user input.

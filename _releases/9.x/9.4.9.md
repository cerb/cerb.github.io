...
jumbotron:
  title: 9.4.9
excerpt: Cerb 9.4.9 is a maintenance update released in January 2020 with 12 minor
  features and fixes from community feedback.
summary: This page details the release notes for Cerb version 9.4.9, a maintenance
  update issued on January 28, 2020. It addresses 12 minor features and fixes based
  on community feedback from the 9.4 update. Key improvements include resolving issues
  with time tracking comments, date range filters, scheduled behaviors, and cache
  file permissions. Additionally, it fixes problems related to importing workspace
  lists, setting ownership in packages, and enhancing user experience in ticket dictionaries
  and bot-powered widgets. The update also ensures proper linking of primary email
  addresses when creating worker records via the API.

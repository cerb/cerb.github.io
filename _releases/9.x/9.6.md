...
jumbotron:
  title: "9.6"
excerpt: Cerb 9.6 is a major feature upgrade released in August 2020 with 73 improvements
  from community feedback.
summary: This page details the release of Cerb 9.6, a feature upgrade launched on
  August 8, 2020, which introduces over 73 new features and improvements based on
  community feedback. Key highlights include the introduction of KATA, a new syntax
  for structured data, enhancements to ticket management with modernized profiles
  and improved message handling, and advanced search capabilities with time limits
  and date range filtering. The update also brings significant improvements to form
  interactions, mailboxes with XOAUTH2 support, and package management with data query
  placeholders. Bots gain new actions like PGP encryption, and dashboards and snippets
  now utilize KATA for better readability and functionality. The release also enhances
  custom fields, connected services with automatic OAuth2 token refreshing, and UI
  elements like a new spinner animation and improved owner picker usability. Additionally,
  platform updates include a per-worker idle timeout feature. The page emphasizes
  the importance of backing up before upgrading and provides compatibility requirements
  for PHP and MySQL.

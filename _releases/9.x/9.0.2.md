...
jumbotron:
  title: 9.0.2
excerpt: Cerb 9.0.2 is a maintenance update released in August 2018 with 10 minor
  features and fixes from community feedback.
summary: This page details the release notes for Cerb version 9.0.2, a maintenance
  update launched on August 21, 2018. It addresses 10 minor features and fixes based
  on community feedback from the 9.0 update. Key improvements include fixes to workspaces
  and dashboards, such as resolving issues with legacy widgets and improving user
  experience by displaying '(no data)' in empty widgets. Other updates involve enhancements
  to profiles and widgets, worklist broadcast functionality, and platform optimization
  to speed up migration processes. Additionally, issues with worker page access, group
  assignment during worker creation, record link displays, and ticket handling in
  the records API have been resolved. The update also includes a new feature on ticket
  profiles that allows users to quickly move tickets to different buckets.

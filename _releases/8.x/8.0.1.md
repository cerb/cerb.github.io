...
jumbotron:
  title: 8.0.1
excerpt: Cerb 8.0.1 is a maintenance update released in June 2017 with 18 minor features
  and fixes from community feedback.
summary: This page details the Cerb 8.0.1 maintenance update released on June 19,
  2017, which includes 18 fixes and minor features based on community feedback from
  the 8.0 update. Key improvements include fixes to mail reply previews, enhancements
  to the global search menu with a 'most frequently used' section, and updates to
  ticket profiles for better participant management and attachment visibility. The
  update also addresses issues with `@mentions`, PHP 7.1 compatibility for community
  portals, and bot interaction loops. Additional features include new options for
  explore mode and ticket profile quick search, as well as improvements to bot-powered
  dashboard widgets and worker worklists. The update aims to enhance usability, fix
  bugs, and streamline user interactions within Cerb.

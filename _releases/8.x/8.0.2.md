...
jumbotron:
  title: 8.0.2
excerpt: Cerb 8.0.2 is a maintenance update released in June 2017 with 15 minor features
  and fixes from community feedback.
summary: This page details the Cerb 8.0.2 maintenance update released on June 30,
  2017, which includes 15 fixes and minor features based on community feedback from
  the 8.0 update. Key improvements include enhancements to bot interactions, such
  as defaulting button prompt colors and improving keyboard navigation. It addresses
  issues with bot behavior variables, custom fields, and workspace page imports. The
  update also fixes problems with event conditions, message header updates, and full-text
  search queries. New features include displaying worker profile pictures in bot behaviors,
  adding a `lastActivity:` filter for worker worklists, and simplifying email address
  selection in mail composition. Additionally, it refines bot interactions and worklist
  actions to improve user experience and functionality.

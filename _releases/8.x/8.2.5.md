...
jumbotron:
  title: 8.2.5
excerpt: Cerb 8.2.5 is a maintenance update released in December 2017 with 17 minor
  features and fixes from community feedback.
summary: This page details the Cerb 8.2.5 maintenance update released on December
  4, 2017, which includes 17 minor features and fixes based on community feedback.
  Key updates include improvements to API handling, such as proper cleanup of API
  credentials when deleting a worker and the ability to set passwords on worker records.
  Enhancements in ticket management include appending `-MERGED` to masks of merged
  tickets to improve forwarding behavior. The update also addresses issues with the
  page wizard, installer packages, and email worklists. Additionally, it fixes several
  bot behavior issues, enhances mail attachment handling, and improves worklist functionalities
  with new subtotaling and filtering options. Security improvements ensure deactivated
  workers cannot use the API with active credentials, and developers can now open
  card popups using record type aliases.

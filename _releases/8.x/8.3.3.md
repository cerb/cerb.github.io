...
jumbotron:
  title: 8.3.3
excerpt: Cerb 8.3.3 is a maintenance update released in March 2018 with 10 minor features
  and fixes from community feedback.
summary: This page details the Cerb 8.3.3 maintenance update released on March 26,
  2018, which includes 10 minor features and fixes based on community feedback from
  the 8.3 update. Key improvements include optimized package importing for large JSON
  files, fixes in the email parser for handling HTML links, and enhancements in the
  JIRA plugin to correctly trigger bot events. The update also addresses API search
  functionality with placeholders, introduces a `content_html` placeholder for message
  records, and enhances bot scripting with improved XML decoding. Additional features
  include a new responsibility filter for ticket worklists, fixes for replying to
  tickets via API and mobile UI, shared worklists for notifications, and the ability
  to select preferred values when merging tickets.

...
jumbotron:
  title: "8.3"
excerpt: Cerb 8.3 is a major feature update released in March 2018 with 72 improvements
  from community feedback.
summary: This page details the release of Cerb 8.3, a feature upgrade launched on
  March 3, 2018, which introduces over 72 new features and improvements based on community
  feedback. Key enhancements include search menu shortcuts, the ability to broadcast
  to any email-based field on records, and an interactive record merge feature. The
  update also supports multi-currency opportunities, revamped worker preferences,
  and built-in portal hosting with friendly URLs and new deploy options. Custom records
  now support profile images, and custom fields have been expanded to include currency
  and decimal-based types with validation. Workspaces have improved usability and
  performance, with new cards and profile pages for various elements. Dashboards now
  offer row and column subtotals on chart widgets and default to three columns. The
  release also addresses security by fixing a content injection issue on the login
  form. The update requires PHP 7.0+ and MySQL 5.6+, and users are advised to back
  up before upgrading.

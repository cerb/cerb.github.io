...
jumbotron:
  title: 8.2.11
excerpt: Cerb 8.2.11 is a maintenance update released in February 2018 with 3 minor
  features and fixes from community feedback.
summary: This page details the Cerb 8.2.11 maintenance update released on February
  28, 2018, which includes three minor features and fixes based on community feedback
  from the 8.2 update. Key improvements include resolving an issue with worker sessions
  using IPv6 addresses in MySQL strict mode, addressing a potential self-XSS vulnerability
  during attachment uploads, and introducing a configuration option to mitigate clickjacking
  by controlling IFRAME embedding through the `APP_SECURITY_FRAMEOPTIONS` setting.

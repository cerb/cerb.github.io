...
jumbotron:
  title: 8.2.6
excerpt: Cerb 8.2.6 is a maintenance update released in December 2017 with 10 minor
  features and fixes from community feedback.
summary: This page details the Cerb 8.2.6 maintenance update released on December
  19, 2017, which includes ten minor features and fixes based on community feedback
  from the 8.2 update. Key improvements include resolving issues with HTML mail templates,
  enhancing bot behaviors and interactions, and adding new bot scripting functions.
  The update also introduces an 'Import Package' action for admin-owned bots, a 'Close
  chat window' action for bot website interactions, and an 'Execute email parser'
  action for processing emails into tickets. Additionally, it addresses maintenance
  scheduler issues related to cleaning up message and draft attachments.

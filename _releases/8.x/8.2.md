...
jumbotron:
  title: "8.2"
excerpt: Cerb 8.2 is a major feature update released in October 2017 with 56+ improvements
  from community feedback.
summary: This page details the release of Cerb 8.2, a feature upgrade launched on
  October 30, 2017, which introduces over 56 new features and improvements based on
  community feedback. Key enhancements include the ability to create custom records
  directly from the browser, advanced management and search capabilities for custom
  fields, and the introduction of reminder records with notification behaviors. The
  update also simplifies mail configuration with group-level defaults and introduces
  new bot capabilities for creating, updating, and managing records. The API has been
  expanded to support abstract operations on all record types, including upserts.
  User interface improvements enhance usability across various components, such as
  the global search menu and editor popups. The release also includes new cards for
  additional record types, streamlined setup processes, and enhanced security features
  for portals. Additionally, popular plugins like conversational bots and project
  boards are now bundled as default features. The update requires PHP 7.0+ and MySQL
  5.6+, and Cerb Cloud subscribers will receive automatic upgrades.

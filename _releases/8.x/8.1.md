...
jumbotron:
  title: "8.1"
excerpt: Cerb 8.1 is a major feature update released in August 2017 with 33+ improvements
  from community feedback.
summary: This page details the release of Cerb 8.1, a feature upgrade launched on
  August 29, 2017, which introduces over 33 new features and improvements based on
  community feedback. Key highlights include enhancements in conversational bots with
  recurrent behaviors, proactive interactions, and file upload prompts. The update
  also supports encrypted email communication using OpenPGP, with features for managing
  public and private keys. Role management has been simplified with itemized permissions
  and dynamic CRUD privileges. Bots have new capabilities for event handling and notification
  management. The release improves workspace performance, introduces new custom field
  types, and enhances the Web API with extended context information and human-readable
  error codes. Developers benefit from a new validation service and improved session
  management for Ajax requests. The page emphasizes the importance of making a backup
  before upgrading and notes that Cerb 8.1 requires PHP 7.0 or higher.

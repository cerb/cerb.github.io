...
jumbotron:
  title: 8.3.6
excerpt: Cerb 8.3.6 is a maintenance update released in April 2018 with 17 minor features
  and fixes from community feedback.
summary: This page details the release notes for Cerb version 8.3.6, a maintenance
  update released on April 16, 2018. The update includes 17 minor features and fixes
  based on community feedback from the 8.3 update. Key improvements include resolving
  a PHP 7.2 warning in bots, enhancing mail composition with better recipient suggestions,
  adding custom fields to saved search records, and improving the installer by running
  the search indexer for initial records. It also addresses issues with merging records,
  enhances contact choosers with email host matching, and allows bulk updates for
  various fields in email address and group records. Additionally, the update provides
  the SHA-1 hash for attachments in API message records, re-enables bulk updates and
  deletions of attachments by admins, updates email relay documentation links, and
  improves user experience on mobile devices for conversational bots.

...
jumbotron:
  title: 8.3.1
excerpt: Cerb 8.3.1 is a maintenance update released in March 2018 with 10 minor features
  and fixes from community feedback.
summary: This page details the Cerb 8.3.1 maintenance update released on March 12,
  2018, which includes 10 minor features and fixes based on community feedback from
  the 8.3 update. Key enhancements include improvements in bot interaction behaviors,
  such as the ability to automatically save user responses to placeholders, specify
  format and validation templates, and simplify the creation of conversational behaviors.
  The update also allows for the import of behaviors for existing bots, enhances the
  'Run behavior' action to work with mixed record list variables, and addresses a
  potential XSS security issue. Additionally, improvements were made to the feedback
  entry records' user experience and bot scripting, including a fix for the `secs_pretty`
  filter and the addition of a method for setting dynamic keys in dictionaries.

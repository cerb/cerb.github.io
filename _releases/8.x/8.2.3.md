...
jumbotron:
  title: 8.2.3
excerpt: Cerb 8.2.3 is a maintenance update released in November 2017 with 26 minor
  features and fixes from community feedback.
summary: This page details the Cerb 8.2.3 maintenance update released on November
  20, 2017, which includes 26 minor features and fixes based on community feedback
  from the 8.2 update. Key improvements address issues with SSL cookies in portals,
  mail relaying with non-local addresses, and various enhancements in bot interactions,
  such as new actions for creating tickets, notifications, and tasks. The update also
  resolves several bugs related to bot behaviors, record handling, and user interface
  issues across different devices and browsers, including Safari and mobile platforms.
  Additionally, the update improves the responsiveness of the global search menu and
  bot chat windows, enhancing usability on mobile devices.

...
jumbotron:
  title: 8.1.4
excerpt: Cerb 8.1.4 is a maintenance update released in October 2017 with 15 fixes
  and minor features from community feedback.
summary: This page details the Cerb 8.1.4 maintenance update released on October 6,
  2017, which includes 15 fixes and minor features based on community feedback from
  the 8.1 update. Key improvements include enhanced autocomplete functionality in
  choosers, more informative tooltips for task status icons, the ability to delete
  fields in custom fieldsets, and corrected database engine selection during installation.
  Additionally, the update allows for calendar and relative date input in ticket worklist
  bulk updates, introduces new file attachment options in bot actions, and improves
  the UI with better autocompletion in the template editor. Other fixes address issues
  with bot scripting, worklist data export, mail composition, and draft previews.

...
jumbotron:
  title: 8.0.5
excerpt: Cerb 8.0.5 is a maintenance update released in August 2017 with 21 minor
  features and fixes from community feedback.
summary: This page details the Cerb 8.0.5 maintenance update released on August 8,
  2017, which includes 21 fixes and minor features based on community feedback from
  the 8.0 update. Key improvements include fixes for issues with the mail compose
  popup, attachment file names in Firefox, negating deep searches, and email address
  handling in the web API. Enhancements were made to the debug status report, worklists,
  and the bot simulator, along with the addition of a StyleCI configuration for developers.
  The update also addresses issues with file bundles, snippets, ticket profiles, and
  compatibility with PHP7. The installer now checks for necessary components like
  the 'vendor' directory and 'mailparse' version.

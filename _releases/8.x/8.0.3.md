...
jumbotron:
  title: 8.0.3
excerpt: Cerb 8.0.3 is a maintenance update released in July 2017 with 22 minor features
  and fixes from community feedback.
summary: This page details the Cerb 8.0.3 maintenance update released on July 17,
  2017, which includes 22 fixes and minor features based on community feedback from
  the 8.0 update. Key improvements include resolving issues with file naming on Windows,
  bot interaction visibility, contact worklist search filters, and ticket worklist
  functionalities. Enhancements were made to contact profile displays, file downloading,
  and bot behaviors, including custom field placeholders and HTTP request execution.
  The update also addresses security concerns with login processes, optimizes performance
  on ticket profiles, and improves error handling in mail parsing. Additionally, new
  features like worker-level comment editing permissions and bot interaction prompt

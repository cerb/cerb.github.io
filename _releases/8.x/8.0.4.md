...
jumbotron:
  title: 8.0.4
excerpt: Cerb 8.0.4 is a maintenance update released in July 2017 with 18 minor features
  and fixes from community feedback.
summary: This page details the Cerb 8.0.4 maintenance update released on July 25,
  2017, which includes 18 fixes and minor features based on community feedback from
  the 8.0 update. Key improvements include enhancements to bots and snippets with
  new message-based placeholders, additional HTTP methods for bot actions, and fixes
  for UI issues such as ticket profiles in iframes and worklist widget editing on
  dashboards. The update also addresses issues with email actions, platform functions,
  and bot behaviors, introduces a British English language pack, and optimizes plugin
  library connections. Additionally, it refines the handling of custom fields and
  notifications related to custom fieldsets.

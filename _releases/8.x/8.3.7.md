...
jumbotron:
  title: 8.3.7
excerpt: Cerb 8.3.7 is a maintenance update released in April 2018 with 12 minor features
  and fixes from community feedback.
summary: This page details the Cerb 8.3.7 maintenance update released on April 27,
  2018, which includes 12 minor features and fixes based on community feedback from
  the 8.3 update. Key improvements include fixes to custom record URLs, project board
  behaviors, and PHP 7.2 compatibility issues. New actions such as 'Create task',
  'Create notification', 'Create ticket', and 'Send email' have been added to custom
  behaviors on custom records. Enhancements also include improved error messaging
  in record validation, a new keyboard shortcut for task profiles, and the introduction
  of an `alias:` filter for search queries on contact, organization, and worker records.

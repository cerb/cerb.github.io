...
jumbotron:
  title: 8.2.1
excerpt: Cerb 8.2.1 is a maintenance update released in November 2017 with 27 minor
  features and fixes from community feedback.
summary: This page details the release notes for Cerb version 8.2.1, a maintenance
  update launched on November 6, 2017. The update includes 27 minor features and fixes
  based on community feedback from the 8.2 update. Key improvements include fixes
  to the 'Send as' setting in mail buckets, enhancements to the API with new fields
  and endpoints for records, and improved validation services for records. The update
  also introduces support for upserts in the API, allowing for efficient record synchronization,
  and restricts API credential creation to administrators. Additionally, it addresses
  issues with time tracking, GPG encryption, and permissions for roles and custom
  records. The update aims to streamline operations and enhance the functionality
  and security of the Cerb platform.

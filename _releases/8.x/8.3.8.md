...
jumbotron:
  title: 8.3.8
excerpt: Cerb 8.3.8 is a maintenance update released in May 2018 with 8 minor features
  and fixes from community feedback.
summary: This page details the release notes for Cerb version 8.3.8, a maintenance
  update released on May 14, 2018. The update addresses eight minor features and fixes
  based on community feedback from the 8.3 update. Key fixes include resolving issues
  with the login form for email addresses containing a plus sign, merging records
  with blank custom fields, bot behaviors running outside their scope, accessing SSL
  resources in portals, incorrect status values in opportunity worklists, subtotaling
  labels in opportunity worklists, and attachment card download issues. Additionally,
  attachment worklists now support filtering by `bundle:` using deep search.

...
excerpt: A step-by-step guide for running large language models on Amazon Bedrock from Cerb.
summary: This page is a step-by-step guide for running large language models on Amazon Bedrock
  from Cerb. Bedrock uses the same AWS keypair and built-in Amazon Web Services connected account
  as any other AWS integration, but it needs its own IAM policy granting the Bedrock invoke and
  list actions plus the AWS Marketplace subscription actions. The guide covers creating that
  policy, attaching it to a user, model access for self-subscribing versus third-party marketplace
  models, choosing a region through the API endpoint URL, understanding cross-region inference
  profile IDs, and creating an agent model record so that automations can reference the model by
  name from the llm.chat, llm.agent, and llm.embed commands.
...
[Amazon Bedrock](https://aws.amazon.com/bedrock/) runs large language models from Anthropic, Amazon, DeepSeek, Meta, Mistral and others on AWS infrastructure, billed through your existing AWS account.
...
Log in to the [AWS Management Console](https://console.aws.amazon.com/iam/) and select the **IAM** service.
...
| Actions | Purpose
|-|-
| `bedrock:InvokeModel`<br>`bedrock:InvokeModelWithResponseStream` | Running a model. Without these, nothing works.
| `bedrock:ListFoundationModels`<br>`bedrock:ListInferenceProfiles` | Lets Cerb ask Bedrock which models your account can actually use, so the model list fills itself in. Without these you have to type model IDs by hand and find out whether they're valid by invoking them.
| `aws-marketplace:ViewSubscriptions`<br>`aws-marketplace:Subscribe` | Lets a model subscribe itself the first time you invoke it, rather than failing until someone visits the console.
...
1. Set **Provider** to **AWS Bedrock**.

1. Set **API endpoint URL** to the Bedrock runtime endpoint for your region (`https://bedrock-runtime.us-east-1.amazonaws.com`). This is how you choose a region -- there's no separate region field. Leave it blank to use `us-east-1`.
  
1. Set **Authentication** to the Amazon Web Services connected account you created above.

1. Click the **refresh** button beside **Model** to load the models your account can use, and pick one. This is what the `ListFoundationModels` and `ListInferenceProfiles` permissions are for.

	<div class="cerb-screenshot">
	<img src="/assets/images/solutions/integrations/aws/agent-model-bedrock.png" class="screenshot">
	</div>
...
1. Picking a model fills in **Name** and sets **Vision** from the provider's own list, so you don't have to know whether a model accepts images. **Context window** is not filled in -- set it yourself from the model's documentation, since compaction ratios are fractions of it.

1. Give the record a short **Name** -- this is what automations will use. Something like `bedrock-kimi` is easier to type than the model ID, and swapping the underlying model later becomes one edit instead of one per automation. Colons aren't allowed, since the name is referenced as `cerb:agent_model:<name>`.

1. Click **Test** to verify the connection, then **Save Changes**.
...
The first is a plain model ID. The leading `global.` and `us.` on the others are **not** part of

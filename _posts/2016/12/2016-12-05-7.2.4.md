---
title: 7.2.4
excerpt: Cerb 7.2.4 is a maintenance update released in December 2016 with 11 fixes
  and minor features from community feedback.
summary: This page details the Cerb 7.2.4 maintenance update released on December
  5, 2016, which includes various fixes and minor features based on community feedback
  from the 7.2 update. Key updates include the addition of contacts to the REST-based
  Web API, improvements to bot scripts with a new 'parse_emails' filter, performance
  enhancements for worklist searches, and several platform-specific fixes for PHP7
  and MySQL 5.7+. The update also addresses issues with community portals, calendar
  event displays, and increases the maximum results for MySQL Fulltext searches. Additionally,
  it introduces an APP_HOSTNAME option for URL generation in specific server environments.
layout: post

...
release:
  type: major functionality update
  count: 143
---
...
[[CHD-4353](https://wgmdev.atlassian.net/browse/CHD-4353)] We've received many requests for the ability to add other built-in and custom fields to the card popups.  Previously, the displayed fields were hardcoded and couldn't be changed without hacking around in the code.
...
[[CHD-4386](https://wgmdev.atlassian.net/browse/CHD-4386)] When viewing an address card popup, a 'Compose' button is now available in the 'Tickets' section. Clicking this buttons opens the compose dialog and pre-fills the email address in the 'To:' field:
...
* [[CHD-4330](https://wgmdev.atlassian.net/browse/CHD-4330)] Contact-based worklists can be subtotaled on gender.
...
* [[CHD-4375](https://wgmdev.atlassian.net/browse/CHD-4375)] In message worklists, added a `header.messageId:` quick search filter for matching messages based on their 'Message-Id:' header.  This is particularly useful for webhook behavior from Mailgun, where the message-id header is the only identifier available.  The filter will only return exact matches.
...
[[CHD-4370](https://wgmdev.atlassian.net/browse/CHD-4370)] Added a primary email address field to organization records.
...
[[CHD-4188](https://wgmdev.atlassian.net/browse/CHD-4188)] Mailbox records now keep tracked of their 'last checked at' timestamp. This is used to check mailboxes in the order of least recently checked first, which addresses issues where a slow or busy first mailbox could block other mailboxes from ever being checked.
...
* [[CHD-4396](https://wgmdev.atlassian.net/browse/CHD-4396)] When sending HTML mail, improved the way that Markdown-formatted lists are converted back to plaintext.

* [[CHD-4329](https://wgmdev.atlassian.net/browse/CHD-4329)] When viewing a ticket timeline, if a message sender has an organization but not a contact, the organization details are now displayed along with a card popup.
...
[[CHD-4226](https://wgmdev.atlassian.net/browse/CHD-4226)] Task records now have a built-in 'importance' field that behaves like the same field on ticket records. This allows for simpler prioritization of tasks without requiring the use of custom fields.
...
[[CHD-4415](https://wgmdev.atlassian.net/browse/CHD-4415)] When opening the 'Create Snippet' popup from compose or reply, any selected text in the current message will be used as the snippet's default text. Previously, this content had to be copied manually in a second step.
...
[[CHD-4266](https://wgmdev.atlassian.net/browse/CHD-4266)] The unread notifications popup now saves any changes made (e.g. filters, columns, subtotals, sorting).
...
[[CHD-4331](https://wgmdev.atlassian.net/browse/CHD-4331)] The 'Worker History over Date Range' and 'Group Replies over Date Range' reports now use the new chooser popups for filtering.
...
[[CHD-4369](https://wgmdev.atlassian.net/browse/CHD-4369) When creating a comment through the Web-API, attachments can now be added to the comment using one or more instances of the `file_id[]` parameter.
...
* [[CHD-3576](https://wgmdev.atlassian.net/browse/CHD-3576)] Implemented bulk update functionality on asset worklists.
...
[[CHD-582](https://wgmdev.atlassian.net/browse/CHD-582)] The installer now automatically adds the client's IP to `AUTHORIZED_IPS_DEFAULTS` in `framework.config.php`.
...
[[CHD-4398](https://wgmdev.atlassian.net/browse/CHD-4398)] Added a global 'Timezone' setting to Setup->Configure->Localization. This serves as the default when a worker doesn't have a timezone set, or there is no session (Virtual Attendants, webhooks, etc). In many standalone deployments this wasn't necessary since the system timezone was sufficient as a default, but in distributed and multi-tenant environments we needed a per-instance default.
...
* [[CHD-4311](https://wgmdev.atlassian.net/browse/CHD-4311)] The `DEVBLOCKS_HTTP_PROXY` option in `framework.config.php` now configures a proxy for all outgoing HTTP requests.  By default this uses an HTTP forward proxy, but the value can be prefixed with `socks5://` to use a SOCKS5 server instead. This allows networked Cerb functionality to work behind a proxy: Virtual Attendant behaviors, Plugin Library, Widget datasources, avatar image URLs, Elasticsearch, etc.

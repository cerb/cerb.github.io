---
title: 7.2.3
excerpt: Cerb 7.2.3 is a maintenance update released in October 2016 with 18 fixes
  and minor features from community feedback.
summary: This page details the release notes for Cerb version 7.2.3, a maintenance
  update launched on October 28, 2016. It includes 18 improvements and fixes based
  on community feedback from the 7.2 update. Key enhancements include the addition
  of an OAuth service to the Devblocks platform, performance improvements in Elasticsearch
  searches, and various bug fixes related to task and ticket ownership, mail reply
  issues, and session management. The update also addresses usability improvements
  in the mail compose interface, error handling in the email parser, and enhancements
  in the REST API for ticket composition. Additionally, it resolves issues with chart
  widgets, virtual attendant scripts, and the maintenance scheduler for attachments.
  The update also reinstates the Setup->Configure->Snippets page for administrators
  and ensures proper handling of custom fields and snippets.
layout: post

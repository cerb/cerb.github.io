---
title: "6.1"
excerpt: Cerb 6.1 is a major feature update released in August 2012 with 51+ new features
...
summary: This page details the release of Cerb version 6.1, a feature upgrade launched
  on August 27, 2012, which introduces over 51 new features and improvements based
  on community feedback. Key enhancements include the introduction of workspaces with
  customizable dashboards featuring widgets, gauges, counters, and charts, as well
  as improved usability with direct links to new records and the ability to add watchers
  when creating records. The update also enhances mail functionalities with response
  time tracking, new activity notifications, and defunct address management. Quick
  search capabilities are improved with hints and new search options, while Virtual
  Attendants gain new behaviors and plugin extensions. The release also includes updates
  for plugin development, platform enhancements, and several maintenance updates (6.1.1
  to 6.1.4) addressing minor fixes and usability improvements. The page emphasizes
  the importance of making a backup before upgrading and provides instructions for
  checking license eligibility and performing the upgrade.
layout: post

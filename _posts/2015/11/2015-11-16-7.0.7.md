---
title: 7.0.7
excerpt: Cerb 7.0.7 is a maintenance update released in November 2015 with 7 fixes
  and minor features from community feedback.
summary: This page details the release notes for Cerb version 7.0.7, a maintenance
  update released on November 16, 2015. The update includes seven improvements based
  on community feedback from the 7.0 update. Key enhancements include improvements
  to the 'Relay to external email' action in Virtual Attendants, fixes for display
  issues with Microsoft Office formatted HTML messages, and corrections for bugs related
  to HTML Mail Templates. Additionally, the update addresses issues with the DevblocksPlatform::strVersionToInt()
  method affecting plugin updates, the display of the 'updated date' field in Ticket
  profiles, and the 'Remove these objects from the variable' option in Virtual Attendants.
  It also resolves an error in exporting custom templates from a portal in newer PHP
  versions.
layout: post

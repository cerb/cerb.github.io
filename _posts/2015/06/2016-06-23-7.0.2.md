---
title: 7.0.2
excerpt: Cerb 7.0.2 is a maintenance update released in June 2015 with 11 fixes and
  minor features from community feedback.
summary: This page details the Cerb 7.0.2 maintenance update released on June 23,
  2015, which includes 11 improvements based on community feedback from the 7.0 update.
  Key enhancements include fixing an updater issue for versions prior to 6.1, resolving
  a MySQL warning when deleting reply-to addresses, and enabling sorting of search
  results by custom fields through the Web-API. The update also addresses installation
  issues on Windows, improves the functionality of custom fields in knowledgebase
  articles, and corrects worker profile display errors. Additionally, it enhances
  dashboard pie chart widgets to properly load data from various formats and refines
  the setup process for worker responsibilities.
layout: post

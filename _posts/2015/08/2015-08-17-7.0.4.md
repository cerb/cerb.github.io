---
title: 7.0.4
excerpt: Cerb 7.0.4 is a maintenance update released in August 2015 with 8 fixes and
  minor features from community feedback.
summary: This page details the release notes for Cerb version 7.0.4, a maintenance
  update issued on August 17, 2015. The update includes eight improvements based on
  community feedback from the 7.0 release. Key enhancements include usability improvements
  for HTML messages in tickets, allowing external links to open in new tabs, and the
  ability for Virtual Attendants to override email headers for more flexible email
  handling. The update also addresses a session activity logging issue when the seat
  count is reached, a PHP notice typo, and a namespace issue in worklist customization.
  Additionally, it resolves a mail sending issue related to 'From:' personal names
  and fixes a medium-risk CSRF vulnerability by implementing a session-based token
  system to prevent unauthorized actions.
layout: post

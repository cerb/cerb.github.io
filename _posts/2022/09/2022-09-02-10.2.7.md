---
title: 10.2.7
excerpt: Cerb 10.2.7 is a maintenance update released in September 2022 with 11 improvements
  from community feedback.
summary: This page details the release notes for Cerb version 10.2.7, a maintenance
  update launched on September 3, 2022. It highlights 11 minor features and fixes
  based on community feedback from the 10.2 update. Key additions include enhancements
  to worker interactions with new options for the `editor:` form element, improvements
  in UI styling for ticket profiles, and updates in automation scripting with a customizable
  delimiter in the `dict_set()` function. Changes include a more user-friendly time
  tracking entry process and improved autocomplete suggestions in record worklists.
  Fixes address issues in email composition, worker interaction toolbars, profile
  image editing, and file handling in automations.
layout: post

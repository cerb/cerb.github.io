---
title: 9.3.3
excerpt: Cerb 9.3.3 is a maintenance update released in August 2019 with 13 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.3.3 maintenance update released on August 28,
  2019, which includes 13 minor features and fixes based on community feedback from
  the 9.3 update. Key improvements include resolving issues with message and comment
  worklists, enhancing user experience for non-authenticated users, and significantly
  improving performance when editing bucket responsibilities. The update also addresses
  issues with attachment records, enhances two-factor authentication leniency, and
  introduces new bot scripting filters for better data normalization. Additionally,
  it improves the display of worker titles in ticket profiles, fixes storage service
  issues, increases the limit for data query categories, and refines the user interface
  for popup cards. The update also includes new unit tests for bot behavior dictionaries.
layout: post

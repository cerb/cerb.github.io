---
title: 9.1.2
excerpt: Cerb 9.1.2 is a maintenance update released in January 2019 with 15 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.1.2 maintenance update released on January 14,
  2019, which includes 15 minor features and fixes based on community feedback from
  the 9.1 update. Key improvements include fixes to dashboard data queries, enhancements
  to the Devblocks string service for handling emojis, and updates to the email parser
  to better manage subjects with emoji characters. The update also introduces new
  functionalities in the records API, such as managing JIRA issues and providing alternative
  worker field options. Usability improvements in bot behaviors, a new scheduled job
  for package imports, and updates to the package exporter script are also highlighted.
  Additionally, the update addresses issues with multi-factor authentication, worklist
  custom fields, and UI display in Firefox, while introducing new search fields for
  filtering by group management in worker and group worklists.
layout: post

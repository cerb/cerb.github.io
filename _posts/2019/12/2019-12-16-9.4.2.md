---
title: 9.4.2
excerpt: Cerb 9.4.2 is a maintenance released in December 2019 with 8 minor features
  and fixes from community feedback.
summary: This page details the Cerb 9.4.2 maintenance update released on December
  16, 2019, which includes eight minor features and fixes based on community feedback
  from the 9.4 update. Key improvements include more intuitive date range filters
  in data and search queries, new expansion keys for email addresses, contacts, and
  organization records to track the most recent messages, and enhancements to the
  user experience with snippet and bot functionalities. The update also addresses
  issues such as the missing 'Profile' button on snippet cards and the relocation
  of the 'Export' option for bots. These changes aim to improve the usability and
  functionality of Cerb for its users.
layout: post

---
title: 9.4.4
excerpt: Cerb 9.4.4 is a maintenance update released in December 2019 with 8 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.4.4 maintenance update released on December
  27, 2019, which includes eight minor features and fixes based on community feedback
  from the 9.4 update. Key improvements focus on user experience enhancements in mail
  composition, replying, and commenting, such as automatic focus on autocomplete menus
  for `#commands` and `@mentions`, and new keyboard shortcuts for sending messages
  and saving comments. Additionally, the update addresses a debugging issue with the
  `/debug/status` endpoint and optimizes autocomplete suggestion delays and text insertion
  for smoother user interaction.
layout: post

---
title: 9.2.1
excerpt: Cerb 9.2.1 is a maintenance update released in March 2019 with 17 minor features
  and fixes from community feedback.
summary: This page details the Cerb 9.2.1 maintenance update released on March 22,
  2019, which includes 17 minor features and fixes based on community feedback from
  the 9.2 update. Key updates include usability improvements for bot behaviors, changes
  to the installer with the replacement of the 'Tutorial' option with a 'Demo' package,
  and adjustments to the nightly maintenance scheduler for notification handling.
  The update also addresses issues with worker records, enhances the package library
  with new workspace and bot action packages, and introduces new scripting functions
  for bots. Additionally, the update includes enhancements to workspace dashboards
  and the addition of a 'Smartsheet' package to the connected service library.
layout: post

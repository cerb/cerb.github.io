---
title: 9.1.5
excerpt: Cerb 9.1.5 is a maintenance update released in February 2019 with 14 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.1.5 maintenance update released on February
  13, 2019, which includes 14 minor features and fixes based on community feedback
  from the 9.1 update. Key improvements include fixes for workspace widgets, bot behaviors,
  login sessions, and custom fields. The update addresses issues such as validation
  errors, session cleanup, memory issues during login, and bot behavior simulations.
  Enhancements also include improved user experience with owner profile pictures on
  worklists, better automation of workflows with custom fieldsets, and usability improvements
  in worker records. Additionally, it resolves issues with bot behaviors related to
  ticket profile views and search filters for inactive workers.
layout: post

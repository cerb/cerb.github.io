---
title: 9.2.3
excerpt: Cerb 9.2.3 is a maintenance update released in May 2019 with 17 minor features
  and fixes from community feedback.
summary: This page details the Cerb 9.2.3 maintenance update released on May 6, 2019,
  which includes 17 minor features and fixes based on community feedback from the
  9.2 update. Key improvements include fixes for calendar syncing issues, enhancements
  to email and comment auto-hyperlinking, and prevention of double-clicking the 'Send
  message' button in mail replies. The update also addresses bot behavior issues,
  optimizes search performance, and improves user experience with tooltips for task
  importance and new configuration options for ticket comments. Additionally, it resolves
  issues with API file uploads, custom field number handling, and knowledgebase article
  dictionaries, while introducing new scripting filters and search filters for ticket
  records.
layout: post

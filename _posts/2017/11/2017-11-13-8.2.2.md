---
title: 8.2.2
excerpt: Cerb 8.2.2 is a maintenance update released in November 2017 with 31 minor
  features and fixes from community feedback.
summary: This page details the Cerb 8.2.2 maintenance update released on November
  13, 2017, which includes 31 minor features and fixes based on community feedback.
  Key updates include enhancements to worklists and search functionalities, such as
  the ability to specify page numbers and result limits in quick search queries, which
  are particularly useful for bots and API interactions. The update also introduces
  several new bot actions for managing records, including create, update, delete,
  upsert, retrieve, and search actions, all of which support custom fields and placeholders.
  Additionally, improvements have been made to knowledgebase article profiles, record
  validation, custom record imports, and workspace page management. The update addresses
  issues with email actions in bots, HTML template previews, and custom record role
  permissions. New quick search filters and usability enhancements for worklists and
  custom records are also included, along with fixes for various interface and functionality
  issues.
layout: post

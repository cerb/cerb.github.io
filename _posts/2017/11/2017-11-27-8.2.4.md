---
title: 8.2.4
excerpt: Cerb 8.2.4 is a maintenance update released on November 27 2017 with 37 minor
  features and fixes from community feedback.
summary: This page details the release notes for Cerb version 8.2.4, a maintenance
  update launched on November 27, 2017. The update includes 37 minor features and
  fixes based on community feedback from the 8.2 update. Key improvements include
  enhancements to the API for package imports, default enabling of the 'Web API' plugin
  in the installer, and various fixes in mail transport settings. The installer has
  been streamlined by removing default group and ticket creations, and simplifying
  the configuration of sender addresses. The update also addresses bugs in bot interactions,
  ticket records, and card displays, while improving usability in group and worker
  management. Additionally, the update refines the process of importing packages,
  enhances project board functionalities, and provides better user experience in ticket
  handling and mail replies. The installer now offers options for setting up either
  a 'Production' or 'Tutorial' environment, with the latter including sample data
  to demonstrate Cerb's capabilities.
layout: post

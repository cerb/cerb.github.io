---
title: 8.1.5
excerpt: Cerb 8.1.5 is a maintenance update released in October 2017 with 10 fixes
  and minor features from community feedback.
summary: This page details the Cerb 8.1.5 maintenance update released on October 16,
  2017, which includes 10 fixes and minor features based on community feedback from
  the 8.1 update. Key improvements involve enhancements to bot functionalities, such
  as adding new actions to recurrent behaviors, fixing issues with message-based bot
  behaviors, and improving date input in conversational interactions. The update also
  addresses issues with importing packages, bot profile images, custom field conditions,
  mail filtering behaviors, and ticket profile shortcuts. Additionally, it introduces
  new targets for 'Before message sent from worker' behaviors, enhancing the use of
  group placeholders and custom fields.
layout: post

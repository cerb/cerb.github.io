---
title: 8.1.6
excerpt: Cerb 8.1.6 is a maintenance update released in October 2017 with 10 fixes
  and minor features from community feedback.
summary: This page details the Cerb 8.1.6 maintenance update released on October 23,
  2017, which includes 10 fixes and minor features based on community feedback from
  the 8.1 update. Key improvements include enhancements in bot scripting with a new
  filter for context ID conversion, fixes in bot interactions and mail filtering behaviors,
  updates to the API for task management and attachment handling, and improvements
  in ticket worklists for bulk updates. The update also addresses issues with bot
  mail actions and ensures server-side cache flushing when importing packages.
layout: post

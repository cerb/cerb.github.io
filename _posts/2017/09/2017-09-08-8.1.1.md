---
title: 8.1.1
excerpt: Cerb 8.1.1 is a maintenance update released in September 2017 with 11 minor
  features and fixes from community feedback.
summary: This page details the Cerb 8.1.1 maintenance update released on September
  11, 2017, which includes 11 fixes and minor features based on community feedback
  from the 8.1 update. Key improvements include security enhancements for mailbox
  access, expanded functionality for bots in email actions, and fixes for package
  import validation issues. The update also addresses SSL validation for community
  portals, improves error messaging for self-signed certificates, and refines email
  handling with separate modifications for sent and saved message versions. Additionally,
  it resolves issues with mail encryption settings, HTML message handling, email parsing
  for S/MIME signatures, bucket editor validation, and webhook triggers for disabled
  bots or behaviors.
layout: post

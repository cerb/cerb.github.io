---
title: 8.1.2
excerpt: Cerb 8.1.2 is a maintenance update released in September 2017 with 18 fixes
  and minor features from community feedback.
summary: This page details the release notes for Cerb version 8.1.2, a maintenance
  update released on September 18, 2017. The update includes 18 fixes and minor features
  based on community feedback from the 8.1 update. Key improvements include fixes
  to calendar synchronization issues, enhancements to the installation test suite,
  and corrections to worklist subtotals when filtered by deleted records. The update
  also addresses bugs related to workspace notifications, improves package personalization
  with a new placeholder dictionary, and enhances the notification cache invalidation
  process. Additionally, it introduces several developer-focused validation service
  enhancements, such as new validators for emails and images, and a formatter for
  context specification. Security improvements include a new IP allowlist option in
  the configuration file, and OAuth-connected accounts now support refresh tokens.
  The update also resolves issues with workspace calendars and dashboard charts, ensuring

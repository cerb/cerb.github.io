---
title: 7.3.6
excerpt: Cerb 7.3.6 is a maintenance update released in March 2017 with 19 fixes and
  minor features from community feedback.
summary: This page details the release notes for Cerb version 7.3.6, a maintenance
  update released on March 20, 2017. The update includes 19 fixes and minor features
  based on community feedback from the 7.3 update. Key improvements include enhanced
  usability in choosers, such as automatic selection of newly added records and resizing
  of toolbar icons. The update also introduces quick search functionality in worklists
  accessed from search buttons in the 'Links:' section. Task profiles now feature
  'Owner' and 'Importance' fields, and task-related bot behaviors have been improved
  to allow setting these fields and properly displaying simulator output. Additional
  fixes address issues in bot scripts, snippets, and actions, including the use of
  placeholders and variables in setting fields. The installer has been updated to
  handle PHP opcache issues and check for the 'mbstring' extension. Notification-related
  issues have also been resolved, ensuring proper evaluation of placeholders and display
  of custom URLs.
layout: post

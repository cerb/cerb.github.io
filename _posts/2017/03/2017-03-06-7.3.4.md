---
title: 7.3.4
excerpt: Cerb 7.3.4 is a maintenance update released in March 2017 with 11 fixes and
  minor features from community feedback.
summary: This page details the Cerb 7.3.4 maintenance update released on March 6,
  2017, which includes 11 fixes and minor features based on community feedback from
  the 7.3 update. Key improvements include fixes to the email address editor, enhancements
  to the API with an added `owner_id` field, and adjustments to worklists and notifications
  to prevent unnecessary refreshes. The update also introduces a new 'Event JSON'
  placeholder for bots, a `reopen:` filter for quick ticket searches, and usability
  improvements in HTML template editing and worker choosers. Additionally, it allows
  for the addition of watchers when creating new tasks and provides an option to update
  comment timestamps during edits.
layout: post

---
title: 7.3.7
excerpt: Cerb 7.3.7 is a maintenance update released on March 27, 2017 with 13 fixes
  and minor features from community feedback.
summary: This page details the Cerb 7.3.7 maintenance update released on March 27,
  2017, which includes 13 fixes and minor features based on community feedback from
  the 7.3 update. Key improvements address issues in various areas such as worklists,
  maintenance tasks, mail parsing, profile links, the Support Center, navigation notifications,
  bots, CAPTCHA configuration, and platform development tools. Notable fixes include
  resolving denial-of-service issues in MySQL, attachment handling, inline image display
  in HTML messages, and bot behavior actions. The update also enhances the functionality
  of the Support Center portal and improves the handling of large HTML messages in
  the email parser. Additionally, it introduces better procedural control for developers
  using the `.cerbPeekTrigger()` jQuery method.
layout: post

---
title: 10.3.9
excerpt: Cerb 10.3.9 is a maintenance update released in May 2023 with 8 improvements
  from community feedback.
summary: This page details the Cerb 10.3.9 maintenance update released on May 08,
  2023, which includes eight minor features and fixes based on community feedback
  from the 10.3 update. Key additions include enhancements in automation scripting
  with new optional arguments for the cerb_placeholders_list() function, a new `comments:`
  search filter for custom records, improved key expandability in `worklist.subtotals`
  data queries, and a profanity filter for ticket mask generation. Fixes address issues
  with package handling when events are disabled, filesystem path detection on Windows,
  performance improvements in activity log searches, and a setup page URL issue.
layout: post

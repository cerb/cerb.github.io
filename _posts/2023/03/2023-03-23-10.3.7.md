---
title: 10.3.7
excerpt: Cerb 10.3.7 is a maintenance update released in March 2023 with 10 improvements
  from community feedback.
summary: This page details the release notes for Cerb version 10.3.7, a maintenance
  update launched on March 23, 2023. It highlights the addition of minor features
  and fixes based on community feedback from the 10.3 update. Key enhancements include
  improvements in metrics and data queries, such as the ability to use negation in
  dimension filters and a new date aggregation option. The update also introduces
  a quick search filter for contact worklists and a validation option in automation
  records. Fixes address issues with bot behaviors in PHP 8.1+, performance improvements
  in data queries, and corrections in the REST API's OAuth2 refresh token process.
  Additionally, several issues with KATA syntax and the automation editor's YAML output
  have been resolved.
layout: post

---
title: 10.4.4
excerpt: Cerb 10.4.4 is a maintenance update released in November 2023 with 23 improvements
  from community feedback.
summary: This page details the release notes for Cerb version 10.4.4, a maintenance
  update launched on November 9, 2023. The update includes 23 minor features and fixes
  based on community feedback from the 10.4 update. Key changes involve enhancements
  to automation interactions, improvements in scheduler and mailbox logging, and formatting
  updates for website interactions. Performance improvements focus on optimizing nightly
  maintenance tasks by eliminating redundant detection of orphaned search index records
  across various data types, such as email addresses, comments, contacts, organizations,
  snippets, tickets, and workers. The update also addresses several bugs, including
  issues with profile widgets, worker interactions, comment notifications, data query
  autocompletion, mail automation, and record maintenance. Additionally, it resolves
  problems related to database cleanup, widget permissions, and the functionality
  of custom fields in record widgets.
layout: post

---
title: 10.4.6
excerpt: Cerb 10.4.6 is a maintenance update released in December 2023 with 9 improvements
  from community feedback.
summary: This page details the release notes for Cerb version 10.4.6, a maintenance
  update released on December 19, 2023. The update includes nine minor features and
  fixes based on community feedback from the 10.4 update. Key additions include enhancements
  to sheet building with new options for link columns, such as icon placement and
  opening links in new tabs. Performance improvements have been made to nightly maintenance
  processes, optimizing ticket record deletions and removing redundant maintenance
  events. Dependency updates have been implemented for better PHP 8.x compatibility
  and security, including updates to `ezyang/htmlpurifier` and `phpseclib/phpseclib`
  in response to a security advisory. Additionally, fixes address issues with PGP
  key-pair generation and record upsertion during installations or upgrades.
layout: post

---
title: 9.0.9
excerpt: Cerb 9.0.9 is a maintenance update released in December 2018 with 11 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.0.9 maintenance update released on December
  14, 2018, which includes 11 minor features and fixes based on community feedback
  from the 9.0 update. Key improvements include enhancements to the Twitter plugin
  for emoji support, fixes for search button issues on cards, and improvements in
  bot actions for multi-checkbox custom fields. The update also addresses issues with
  portal hosting behind proxies, introduces new placeholders for mail filtering bot
  behaviors, and resolves problems with deleting misconfigured widgets on profiles.
  Additionally, it fixes usability issues with Firefox menus, corrects worker record
  deletion processes, and resolves long-standing issues in bot behaviors related to
  worker-based conditions and multi-checkbox custom fields.
layout: post

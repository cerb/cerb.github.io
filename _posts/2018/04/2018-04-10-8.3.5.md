---
title: 8.3.5
excerpt: Cerb 8.3.5 is a maintenance update released in April 2018 with 9 minor features
  and fixes from community feedback.
summary: This page details the Cerb 8.3.5 maintenance update released on April 10,
  2018, which includes nine minor features and fixes based on community feedback from
  the 8.3 update. Key improvements include fixes to the bot interaction chat window,
  enhancements to the API for splitting and merging tickets, and adjustments to ticket
  merging to prevent multiple '-MERGED' suffixes. The update also addresses issues
  with notification URLs, bot behaviors related to ticket ownership, and compatibility
  with Elasticsearch 6.x+ versions. Additionally, it resolves a PHP 7.2 warning on
  watcher buttons in worklists and improves the handling of binary content in bot
  HTTP requests.
layout: post

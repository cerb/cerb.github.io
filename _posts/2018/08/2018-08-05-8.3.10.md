---
title: 8.3.10
excerpt: Cerb 8.3.10 is a maintenance update released in August 2018 with 2 minor
  features and fixes from community feedback.
summary: This page details the Cerb 8.3.10 maintenance update released on August 5,
  2018, which includes two minor features and fixes based on community feedback from
  the 8.3 update. The update addresses issues with custom records where the owner
  menu was not appearing when records were configured to be ownable only by groups
  or roles. It also fixes a problem with ticket worklists during bulk updates, where
  a fulltext filter with many results could prevent the 'only checked' option from
  modifying selected records. Instructions for upgrading are provided.
layout: post

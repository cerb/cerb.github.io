---
title: 9.0.8
excerpt: Cerb 9.0.8 is a maintenance update released in November 2018 with 11 minor
  features and fixes from community feedback.
summary: This page details the release notes for Cerb version 9.0.8, a maintenance
  update released on November 23, 2018. The update includes 11 minor features and
  fixes based on community feedback from the 9.0 update. Key improvements include
  enhancements to bot actions and scripting, such as the addition of a `|base_convert(from,to)`
  filter and improvements in handling custom fields and HTTP requests. The update
  also addresses issues with package imports, search query syntax, and ticket worklists.
  Additionally, changes in the JIRA plugin allow for better synchronization and management
  of JIRA projects within Cerb, including the ability to create and edit JIRA project
  records directly from Cerb.
layout: post

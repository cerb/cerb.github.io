---
title: 8.3.4
excerpt: Cerb 8.3.4 is a maintenance update released in March 2018 with 11 minor features
  and fixes from community feedback.
summary: This page details the Cerb 8.3.4 maintenance update released on March 30,
  2018, which includes 11 minor features and fixes based on community feedback from
  the 8.3 update. Key enhancements include improvements in package handling, such
  as making all record placeholders available when chooser keys end with `_id`, and
  allowing blank fields for message and ticket imports. Security is enhanced with
  the addition of the `noopener` flag to outbound links to prevent tabnabbing. Bots
  have been improved with new conditions and prompt functionalities, including deep
  searching capabilities for ticket and message records, and enhanced file upload
  prompt handling. The update also addresses validation processes in conversational
  bots, ensuring placeholders are cleared upon validation failure.
layout: post

---
title: 8.3.2
excerpt: Cerb 8.3.2 is a maintenance update released in March 2018 with 19 minor features
  and fixes from community feedback.
summary: This page details the Cerb 8.3.2 maintenance update released on March 21,
  2018, which includes 19 minor features and fixes based on community feedback from
  the 8.3 update. Key improvements include enhanced error messaging for email replies,
  fixes for record and ticket merge permissions, and security enhancements such as
  sanitized HTML on project boards and improved CSRF protection. The update also addresses
  issues with bot interactions, workspace exports, and attachment handling, while
  introducing new functionalities like configurable input parameters for bot-powered
  dashboard widgets and a script for exporting tickets as packages. Additionally,
  it provides developers with tools to allow Cerb's HTML5 markup while stripping disallowed
  content.
layout: post

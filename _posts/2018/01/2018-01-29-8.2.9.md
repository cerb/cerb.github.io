---
title: 8.2.9
excerpt: Cerb 8.2.9 is a maintenance update released in January 2018 with 9 minor
  features and fixes from community feedback.
summary: This page details the Cerb 8.2.9 maintenance update released on January 30,
  2018, which includes nine minor features and fixes based on community feedback from
  the 8.2 update. Key improvements include validation for the 'Files' custom field
  type, a fix for the AWS plugin's request signature validation issue, enhancements
  in dictionary key retrieval for developers, and resolution of a validation issue
  with 64-bit numbers. Additionally, dashboard chart widgets now correctly apply format
  options, knowledgebase article categories are clickable, and the 'updated' date
  no longer changes with view count increases. Other fixes address issues with calendar
  worklists and PHP warnings in portal template exports.
layout: post

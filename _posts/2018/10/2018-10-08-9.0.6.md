---
title: 9.0.6
excerpt: Cerb 9.0.6 is a maintenance update in development during October 2018 with
  8 minor features and fixes from community feedback.
summary: This page details the Cerb 9.0.6 maintenance update released on October 8,
  2018, which includes eight minor features and fixes based on community feedback
  from the 9.0 update. Key improvements include a new worker preference for email
  reply behavior, a fix for the mail reply form's watcher button, enhancements in
  data queries involving custom fields, and adjustments in worklist customization
  and search functionalities. Additionally, bot scripting now suggests the `cerb_placeholders_list()`
  function, and ticket profiles have a relocated permalink button for better user
  experience. The update also addresses issues with currency and decimal custom fields
  in worklists and data queries.
layout: post

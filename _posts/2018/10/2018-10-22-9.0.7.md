---
title: 9.0.7
excerpt: Cerb 9.0.7 is a maintenance update released in October 2018 with 22 minor
  features and fixes from community feedback.
summary: This page details the release notes for Cerb version 9.0.7, a maintenance
  update launched on October 22, 2018. The update addresses 22 minor features and
  fixes based on community feedback from the 9.0 update. Key improvements include
  resolving issues with email drafts containing Emoji characters, task importance
  default settings, custom field displays on cards, project board queries, and worklist
  subtotals. Enhancements also cover user experience improvements on project boards,
  support center profile updates, LDAP authentication, and workspace profile widgets.
  Additionally, the update introduces new tools for exporting ticket data and improves
  performance in bot dictionaries and worklist exports. The update also refines data
  query functionalities, allowing for more complex mathematical operations and aggregations
  in worklist subtotals.
layout: post

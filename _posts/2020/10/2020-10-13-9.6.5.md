---
title: 9.6.5
excerpt: Cerb 9.6.5 is a maintenance update released in October 2020 with 8 minor
  features and fixes from community feedback.
summary: This page details the release notes for Cerb version 9.6.5, a maintenance
  update issued on October 13, 2020. The update addresses eight minor features and
  fixes based on community feedback from the 9.6 update. Key improvements include
  visibility of attachments on custom records for non-admin users, enhancements in
  bot behavior conditions for phone fields, preservation of search queries through
  the login process, and improved UX for `@mentions` in comments. Additionally, the
  update introduces XOAUTH2 authentication for SMTP connections, allows comments in
  the Data Query Tester, and corrects notification permalink generation for custom
  records. These changes aim to enhance functionality and user experience across various
  components of Cerb.
layout: post

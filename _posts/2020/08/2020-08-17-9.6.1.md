---
title: 9.6.1
excerpt: Cerb 9.6.1 is a maintenance update released in August 2020 with 8 minor features
  and fixes from community feedback.
summary: This page details the Cerb 9.6.1 maintenance update released on August 17,
  2020, which includes eight minor features and fixes based on community feedback
  from the 9.6 update. Key issues addressed include a fix for bot behavior loop nodes
  that caused infinite loops, improvements to handling non-standard IMAP mail servers
  to prevent message duplication, and corrections for processing malformed HTML in
  emails. Additionally, enhancements were made to ticket profile warnings, organization
  record search filters, card popup editors, workspace interaction widgets, and UI
  styling for comment threads. The update aims to improve functionality and user experience
  across various components of Cerb.
layout: post

---
title: 9.5.5
excerpt: Cerb 9.5.5 is a maintenance update released in April 2020 with 5 minor features
  and fixes from community feedback.
summary: This page details the Cerb 9.5.5 maintenance update released on April 30,
  2020, which includes five minor features and fixes based on community feedback from
  the 9.5 update. Key improvements include resolving issues with text columns in sheets,
  enhancing usability on notification worklists by recalculating the last page when
  non-first pages are empty, fixing the 'Mention Name' issue on worker cards, improving
  the usability of the suggestions menu in worklist search queries to prevent interference
  with typing, and addressing a problem with replying to tweets from their card popup.

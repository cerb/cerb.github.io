---
title: 9.4.6
excerpt: Cerb 9.4.6 is a maintenance update released in January 2020 with 8 minor
  features and fixes from community feedback.
summary: This page details the Cerb 9.4.6 maintenance update released on January 6,
  2020, which includes eight minor features and fixes based on community feedback
  from the 9.4 update. Key improvements include adjustments to the autocomplete shortcut
  to avoid conflicts with macOS/Safari's emoji picker, fixes for saving changes to
  multi-checkbox custom fields, enhancements to `@mention` autocompletion, and resolving
  issues with the 'Save without sending' option in mail replies. Additionally, the
  update allows for better handling of multi-checkbox fields during CSV imports, provides
  more control over sender details in broadcasts, enables bot behaviors to override
  scheduled sending times, and introduces a 'Participants' column in ticket worklists

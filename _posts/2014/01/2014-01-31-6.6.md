---
title: "6.6"
excerpt: Cerb 6.6 is a major feature update released in January 2014 with 70+ new
...
summary: This page details the release of Cerb version 6.6, a feature upgrade launched
  on January 31, 2014, which includes over 70 new features and improvements based
  on community feedback. Key enhancements include the ability to send HTML formatted
  messages, improved mail relay functions, and the introduction of custom fields for
  owners and record links. The update also enhances storage capabilities with automatic
  duplicate file detection and bulk deletion support, and improves the Web-API with
  new functionalities for listing, linking, and unlinking records. Usability improvements
  are made across various components such as the Support Center, Explore Mode, and
  the user interface. The release also includes four maintenance updates (6.6.1 to
  6.6.4) that address minor fixes and usability enhancements. The page provides detailed
  instructions for upgrading and checking license eligibility for the update.
layout: post

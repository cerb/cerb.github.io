---
title: 'Metrics: Agent Model Output Tokens'
excerpt: This page provides examples and details on the cerb.agent.model.tokens.output metric
permalink: /docs/metrics/cerb.agent.model.tokens.output/
toc:
  title: cerb.agent.model.tokens.output
jumbotron:
  title: cerb.agent.model.tokens.output
  tagline: ~
  breadcrumbs:
...
This counter tracks **completion tokens** returned by a model on each [agent](/docs/agents/) turn, broken down by model, agent, and the worker the turn ran for.

Reasoning tokens are **not** counted separately. Providers report them as part of the output total, so a second series would double-count any bare sum across these metrics. A turn's reasoning token count is still shown individually on its [transcript](/docs/setup/developers/llm-agent-transcripts/).

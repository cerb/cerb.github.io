...
summary: This page provides an overview of the "while" command in Cerb automations,
  ...
  the syntax for the "if" and "do" keys, explaining that "if" must resolve to a boolean
  value to determine whether the loop continues, and "do" contains the commands to
  be repeated. It lists the values treated as false, notes that every other value is
  true, and warns that a condition rendering to arbitrary text loops until the
  automation reaches its time limit.
...
A comparison does that: Twig renders `true` as `1` and `false` as empty text, so {% raw %}`{{counter < 5}}`{% endraw %} alternates between a value that's `true` and one that's `false`.

A bare placeholder doesn't. Any text that isn't in that list is `true`, so {% raw %}`if: {{my_flag}}`{% endraw %} keeps repeating until the automation reaches its [time limit](/docs/automations/#time-limit), which defaults to 25 seconds. Nothing reports that the condition was never a boolean.

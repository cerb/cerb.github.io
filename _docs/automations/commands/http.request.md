...
<p class="youtube-video-container"><iframe width="1280" height="720" src="https://www.youtube.com/embed/3yn_WWPzAoU" title="A deep dive into the http.request automation command" frameBorder="0"  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen></iframe></p>
...
The **HTTP Basic Authentication** and **API Key** providers only sign requests to the hosts configured on their [connected service](/docs/records/types/connected_service/). A request to any other host fails to authenticate and runs the [on_error:](#on_error) event rather than sending an unauthenticated request.
...
Redirects are followed automatically, up to five of them, for `http` and `https` only. Exceeding that limit runs the [on_error:](#on_error) event.

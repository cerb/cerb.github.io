...
excerpt: The automation behind an agent tool record -- an AI agent calls the tool, this runs, and its content is what the model reads back.
...
permalink: /docs/automations/triggers/agent.tool/
toc:
  title: agent.tool
  expand: Automations
jumbotron:
...
| Key                    | Type       | Notes                                                                                                                                                                            |
|------------------------|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `inputs.*`             | dictionary | The arguments for this call, read as {% raw %}`{{inputs.query}}`{% endraw %}. Declared in this automation's own `inputs:` block                                                    |
...
<div class="cerb-box note">
	<p>
		A tool that genuinely needs to ask the worker something -- an approval, a missing argument,
		a handoff to another interaction -- belongs to the calling script instead. Leave the
		<a href="/docs/records/types/agent_tool/">agent tool</a> record's automation empty and answer
		the call from the conversation's own <code>on_tool:</code> branch with
		<code>tool.return:</code>, which <i>can</i> await.
	</p>
</div>

# Starter template

**Automations &raquo; Build &raquo; AI Agent Tool** seeds a working script on this trigger, together with the [agent tool](/docs/records/types/agent_tool/) record that calls it.

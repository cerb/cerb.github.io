...
<div class="cerb-box warning">
	<p>
		The launcher passes the agent as a <b>reserved</b> input named <code>agent</code>, consumed
		before your <code>inputs:</code> are validated. An automation that declares
		<code>agent</code> in its own <code>inputs:</code> block fails to run with
		<b>Unknown inputs: agent</b>. Read <code>agent_*</code> from scope instead.
	</p>
</div>
...
<div class="cerb-box warning">
	<p>
		<b>Leave <code>callers:</code> unset.</b> An automation with no <code>callers:</code> block
		allows every caller, which is what an agent chat wants. Naming only <code>agent.pane</code>
		is the tempting mistake -- it loses the command bar and breaks resuming the chat from
		History. A <code>callers:</code> block here has to allow <b>both</b> names.
	</p>
</div>
...
| `grepField`     | `cerb_grep_routing`  | `query:`; `limit:` optional, default 25                       | JSON `[{line, path, text}]` -- line numbers *and* the KATA key path for each hit, so a location never has to be guessed.                                                                            |

...
excerpt: This page provides detailed information about Agent Tool records in Cerb, including their structure and usage within the system.
...
permalink: /docs/records/types/agent_tool/
toc:
  title: Agent Tool
  expand: Records
jumbotron:
...
<div class="cerb-box note">
	<p>
		<b>A tool holds no credentials.</b> There is no key or secret field on the record. An API key
		for whatever the tool calls belongs to the
		<a href="/docs/automations/triggers/agent.tool/">automation</a> behind it, or to a
		<a href="/docs/records/types/connected_account/">connected account</a> that automation reads.
	</p>
</div>
...
The `icon` must be a [cerb-icons](/docs/developers/icons/) name, or saving is refused with *"must be a cerb-icons name."* Leave it blank rather than inventing one.
...
<div class="cerb-box note">
	<p>
		So labels are worth writing, but leaving them blank is not a failure -- it costs you the
		phrasing, not the meaning. A reader still learns which tool ran and how long it took.
	</p>
</div>
...
<div class="cerb-box note">
	<p>
		To <b>force</b> an argument the model shouldn't get to choose, rewrite
		<code>__tool.parameters</code> in the calling script's <code>on_tool:</code> branch, which runs
...
* A tool that's disabled, or whose record is missing, when a conversation *starts* is never admitted at all -- so the model is never shown something it would only be refused.

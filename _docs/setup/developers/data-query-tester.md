...
excerpt: This page shows how to access and use the Data Query Tester in the developer menu.
...
permalink: /docs/setup/developers/data-query-tester/
toc:
  expand: Admin Guide
jumbotron:
  title: Data Query Tester
...
<img src="/assets/images/docs/setup/data-query-tester.png" class="screenshot">
</div>
...
It highlights **two grammars at once**, which is what a data query actually is: the query syntax itself, and any [scripting](/docs/scripting/) tags embedded in it. Tag state carries across lines, so a multi-line {% raw %}`{% ... %}`{% endraw %} block keeps its highlighting.
...
<div class="cerb-box note">
	<p>
		<b>Errors appear in the results panel, not as an alert.</b> A bad <code>type:</code>, an unknown
		field, or a missing <code>type:</code> comes back as an <code>error</code> object, and a broken
		scripting tag comes back as a quoted message. Either way the place to look is the same one you
		read results in.
	</p>
</div>
...
<div class="cerb-box note">
	<p>
		<b>The agent can neither run the query nor see the results.</b> There's no run command in that
		list, and no command that returns output -- the results panel is only ever filled by a person
		clicking <b>Run</b>. An agent that needs to know what a query returns evaluates one of its own
		server-side and tells you in the chat, rather than driving the screen and reading it back.
	</p>
</div>

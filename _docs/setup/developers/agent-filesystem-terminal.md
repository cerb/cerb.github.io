...
<img src="/assets/images/docs/setup/agent-filesystem-terminal.png" class="screenshot">
</div>
...
<div class="cerb-box note">
	<p>
		The <code>ro</code>/<code>rw</code> switch is a workflow safeguard, not a security boundary
		-- it protects you from a mistyped <code>rm</code>, not from a determined operator. Access
		control for this screen is the administrator requirement itself.
	</p>
</div>
...
<div class="cerb-box note">
	<p>
		<b>Everything here lives in your browser.</b> The mount set, the working directory, and the
		<code>/tmp</code> scratch area are held in the page, and the server keeps no state between
		commands. Reloading -- or navigating away and back -- returns you to an empty terminal
		with nothing mounted, without warning. Nothing you do here is lost from the volumes themselves;
		it's the session setup that goes.
	</p>
</div>
